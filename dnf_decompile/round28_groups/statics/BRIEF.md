# statics 第 28 轮类粒度回归 —— 子 Agent 通用手册（2026-08-12）
...
- ORIG 二进制：`/home/loyieking/dnf_workspace/dnf_installer/build/dnf_data/home/template/neople/statics/df_statics_r`
  ...
  4.4.7-1 / 4.4.4-13 混合）。
- 权威重建：`build/statics/df_statics_r`（第 28 轮起统一 **c6446r** 变体：
  c6root g++ 4.4.7 驱动 + 4.4.6 cc1plus，`-m32 -O0 -std=gnu++0x`；
  注意任务里“c6”口径经 A/B 实测：c6(4.4.7) 全量重建为 125 md，c6446r 为 70 md，
  故本轮以 c6446r 为准，与 `cmake_build_all.sh` 的 statics 映射一致）。
- 权威基线：statics 非 identical = **70**（DIFF 68 + NEAR 2；IDENTICAL 554 /
  IDENTICAL_AE 219）。你组清单见 `round28_groups/statics/<group>.list`
  （mangled 名，一行为一个）。
...
| gm_accounts | `.../GMAccounts.cpp` | 6 | 独占 TU |
...
3. 重编 + 链接 scratch 二进制（只写你自己的 scratch 目录）：
   ...
   - **私有副本组**（stats_lifecycle/stats_add/stats_write/stats_senddb_a/
     ...
     scratch_link <group>
     ```
   - 逐 TU 工具链 A/B（仅当怀疑某函数 ORIG 编译器版本与 c6446r 不符）：
...
4. 复检：
   ...
   # 批量复检整组：
   python3 source/toolchain/statics_scratch_check.py /tmp/sub_statics_<group>/df_statics_r --file round28_groups/statics/<group>.list
   ```
...
- **禁止硬套内联 asm**（唯一例外：纯 `nop` 对齐填充）。到头函数按 §7 上报，
  不得用 asm 硬套。
- 禁止清空/删除整文件实现。
