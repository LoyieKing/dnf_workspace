# statics 第 22 轮类粒度回归 —— 子 Agent 通用手册（2026-08-12）
...
- 权威基线（2026-08-12 重新 manifest+md 全量测量）：statics 非 identical = 75
  （DIFF 71 + NEAR 4；IDENTICAL 550 / IDENTICAL_AE 218）。
...
2. 改源码（只改你组函数所属的 TU；规则见 §4）。
3. 重编 + 链接 scratch 二进制（只写你自己的 scratch 目录）：

   ```bash
   bash source/toolchain/statics_scratch.sh <scratch_dir> <TU basename>
   # 例：bash source/toolchain/statics_scratch.sh /tmp/statics_g_add Statistics
   ```

   - TU basename 不带 `.cpp`。
   - 私有副本编译（仅 Statistics.cpp 各组）：先把
     `/tmp/statics_base/Statistics.cpp` 复制成自己的副本，编辑副本，
     然后 `export STATICS_SCRATCH_SRC_OVERRIDE=<副本路径>` 再跑上面的命令。
4. 复检：
...
- 本轮开始前已有 10 个函数因源码形态 identical（CUdpHandler×4、CApplication×2、
  CGMAccounts::AppendGM_Sys、CFrameCountHandler::GetFrameCountInfo、
  CHWSpecResearcher×2）——说明剩余差异绝大多数是 -O0 代码生成
  （栈槽/寄存器/求值顺序/寻址形态），源码形态可消除。
- 典型小 diff：`mov %ebx,-0x4c(%ebp)` vs `mov %edx,-0x4c(%ebp)`（寄存器名）；
  两条 `mov` 交换（求值顺序）；`movw $0x0,0xc(%eax,%edx,2)` vs
  `add %edx,%edx; add %edx,%eax; movw $0x0,(%eax)`（寻址形态）。
