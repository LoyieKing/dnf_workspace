# guild 第 32 轮类粒度回归 —— 子 Agent 通用手册（2026-08-12）
...
## 1. 背景与权威基线（2026-08-12，第 32 轮）

- 工作区根：`/home/loyieking/dnf_workspace/dnf_decompile`（所有路径以此为准）。
- ORIG 二进制：
  `/home/loyieking/dnf_workspace/dnf_installer/build/dnf_data/home/template/neople/guild/df_guild_r`
...
- 权威重建：`build/guild/df_guild_r`（guild 用 c6 变体：c6root g++ 4.4.7 驱动，
  ...
  `DNFGuildServerMain`/`TcpNetSystem` 默认走 c6444r，见
  `source/toolchain/cmake/dnf_guild_cxx.sh`；`DNFUser.cpp` 与
  `CFileLogWriterBase.cpp` 带 `-fno-exceptions`）。
- **第 32 轮权威基线（父 agent 已从 HEAD 全新重建并先 manifest 再全量重生成）**：
  guild 非 identical = **266**（DIFF 249 + NEAR 17；IDENTICAL 963 /
  IDENTICAL_AE 321）。工作树干净（无未提交改动）。
- 你组清单见 `round32_groups/guild/<group>.list`（mangled 名，一行为一个）。
- 判定口径：`source/toolchain/compare_common.py` strict（仅归一化直接跳转/调用
  目标地址；立即数、字段/栈偏移、寄存器、寻址形态、数据地址严格保留）。
...
3. 重编 + 链接 scratch 二进制（只写你自己的 scratch 目录）：
   ...
   - 私有副本组（pt_a/pt_b/pt_c/guild_a/guild_b，同 TU 多组并行）：
     ...
     cp /tmp/guild_base_r32/<TU>.cpp /tmp/sub_guild_<group>/private/<TU>.cpp
     # 编辑 /tmp/sub_guild_<group>/private/<TU>.cpp
     export GUILD_SCRATCH_SRC_OVERRIDE=/tmp/sub_guild_<group>/private/<TU>.cpp
     scratch_compile <group> /home/loyieking/dnf_workspace/dnf_decompile/source/DNFServer/GameServer/Guild/<TU>.cpp
     scratch_link <group>
     ```
     `/tmp/guild_base_r32/*.cpp` 已由父 agent 在开工时刷新为当前 HEAD（含第 31 轮
     修复），私有副本务必从 `/tmp/guild_base_r32` 复制。
...
2. 你的源码改动 diff：
   ...
   - 私有副本组：`diff -u /tmp/guild_base_r32/<TU>.cpp /tmp/sub_guild_<group>/private/<TU>.cpp`
3. 需要父 agent 决策的共享改动（头文件/布局/工具链 A/B 结论）。

## 7. 分组与 TU 归属（round32_groups/guild/*.list，由第 32 轮权威 manifest 生成）

| 组 | 函数数 | TU（basename） | 私有副本 |
|---|---|---|---|
| pt_a / pt_b / pt_c | 32/32/31 | DNFPacketTranslater | 是（每组一份） |
| guild_a / guild_b | 29/28 | DNFGuild | 是（每组一份） |
| guildmgr | 13 | DNFGuildManager | 否（独占） |
| srvhdr | 7 | DNFServerHandler | 否（独占） |
| user_peer | 9 | DNFUser + Peer | 否（独占） |
| cargo_board | 13 | GuildCargo + GuildBoard | 否（独占） |
| usermgr_powermgr | 14 | DNFUserManager + PowerManager | 否（独占） |
| war_family | 13 | DNFGuildWar + PowerWarGuildInfo + PowerWar + PowerWarCharacInfo | 否（独占） |
| decoder_udp | 12 | DNFUdpHandler + DNFPacketDecoder + DNFTcpHandler | 否（独占） |
| xml_frame_sig | 23 | ServerXml + DNFTickHandler + DNFSignalTranslator + DNFTableBase + DNFApplication | 否（独占） |
| small_tu | 10 | DNFAppStartInit + DNFAppStopInit + DNFTcpSocket + MemoryCashManager + GMAccounts + DNFTcpNetworkThread + DNFNetworkThread + CashObject + TcpNetSystem | 否（独占） |
...
- 剩余 266 个绝大多数是 -O0 代码生成差异（栈槽/寄存器/求值顺序/寻址形态/
...
- scratch 辅助脚本已修复 zsh 兼容（`$FLAGS` 显式分词）；直接
  `source source/toolchain/guild_scratch_helper.sh` 即可，bash/zsh 均可。
