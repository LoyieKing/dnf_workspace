# _ZN13CGuildManager22CheckAchieveAttendanceEj

`CGuildManager::CheckAchieveAttendance(unsigned int)`

| 服务 | 状态 | ORIG 地址 | ORIG 大小 | 重建地址 | 重建大小 |
|---|---|---|---|---|---|
| guild | DIFF | `0x8097450` | `0xa7` | `0x805d4d0` | `0xa9` |
...
@@ -1,51 +1,52 @@
 ...
 test   %al,%al
-je     <T> <_ZN13CGuildManager22CheckAchieveAttendanceEj+0xa0>
+je     <T> <_ZN13CGuildManager22CheckAchieveAttendanceEj+0xa2>
 movl   $0x8,-0xc(%ebp)
-jmp    <T> <_ZN13CGuildManager22CheckAchieveAttendanceEj+0x94>
+jmp    <T> <_ZN13CGuildManager22CheckAchieveAttendanceEj+0x96>
 lea    -0x14(%ebp),%eax
 mov    %eax,(%esp)
...
+mov    %eax,%edx
+mov    -0xc(%ebp),%eax
+mov    &_ZZN13CGuildManager22CheckAchieveAttendanceEjE15guild_att_phase(,%eax,4),%eax
+cmp    %eax,%edx
 sete   %al
 test   %al,%al
-je     <T> <_ZN13CGuildManager22CheckAchieveAttendanceEj+0x90>
+je     <T> <_ZN13CGuildManager22CheckAchieveAttendanceEj+0x92>
 mov    -0xc(%ebp),%eax
-jmp    <T> <_ZN13CGuildManager22CheckAchieveAttendanceEj+0xa5>
+jmp    <T> <_ZN13CGuildManager22CheckAchieveAttendanceEj+0xa7>
 subl   $0x1,-0xc(%ebp)
 mov    -0xc(%ebp),%eax
 not    %eax
 shr    $0x1f,%eax
 test   %al,%al
 jne    <T> <_ZN13CGuildManager22CheckAchieveAttendanceEj+0x62>
 mov    $0xffffffff,%eax
 leave
 ret
```
...
int CGuildManager::_ZN13CGuildManager22CheckAchieveAttendanceEj(uint param_1)
  ...
  if (cVar1 != '\0') {
    for (local_10 = 8; -1 < local_10; local_10 = local_10 + -1) {
      iVar2 = std::
              _Rb_tree_iterator<std::pair<unsigned_int_const,std::vector<unsigned_int,std::allocator<unsigned_int>>>>
              ::operator->(local_18);
      iVar2 = std::vector<unsigned_int,std::allocator<unsigned_int>>::size
                        ((vector<unsigned_int,std::allocator<unsigned_int>> *)(iVar2 + 4));
      if (iVar2 == *(int *)(::guild_att_phase + local_10 * 4)) {
        return local_10;
      }
    }
  }
...
定义于 [source/DNFServer/GameServer/Guild/DNFGuildManager.cpp](source/DNFServer/GameServer/Guild/DNFGuildManager.cpp)（约第 703 行）：
