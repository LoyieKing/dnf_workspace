...
|---|---|---|---|---|---|
| guild | DIFF | `0x807e1e2` | `0x2c5` | `0x8074a50` | `0x2e8` |
...
-mov    %eax,0x4(%esp)
-mov    %edx,(%esp)
+jmp    <T> <_ZN17CPacketTranslater25OnDBMWResisterToBlackListEP12PacketHeader+0x2e1>
+mov    -0x14(%ebp),%eax
+add    $0xa,%eax
+mov    (%eax),%ebx
+mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
...
+mov    %eax,-0x10(%ebp)
+cmpl   $0x0,-0x10(%ebp)
+jne    <T> <_ZN17CPacketTranslater25OnDBMWResisterToBlackListEP12PacketHeader+0xb7>
 movl   $0xf65,0x8(%esp)
 movl   $"OnDBMWResisterToBlackList",0x4(%esp)
-lea    -0x2c(%ebp),%eax
+lea    -0x24(%ebp),%eax
...
+mov    (%eax),%eax
+cmp    $0xffffffff,%eax
+jne    <T> <_ZN17CPacketTranslater25OnDBMWResisterToBlackListEP12PacketHeader+0x12a>
+lea    -0x65(%ebp),%eax
+add    $0x30,%eax
+movb   $0x3,(%eax)
+lea    -0x65(%ebp),%eax
...
+sete   %al
+test   %al,%al
+je     <T> <_ZN17CPacketTranslater25OnDBMWResisterToBlackListEP12PacketHeader+0x1e5>
+mov    -0x10(%ebp),%eax
+mov    %eax,(%esp)
...
+setbe  %al
+test   %al,%al
+je     <T> <_ZN17CPacketTranslater25OnDBMWResisterToBlackListEP12PacketHeader+0x1bd>
+mov    -0x14(%ebp),%eax
+lea    0xe(%eax),%edx
+mov    -0x14(%ebp),%eax
+add    $0x2c,%eax
...
+jne    <T> <_ZN17CPacketTranslater25OnDBMWResisterToBlackListEP12PacketHeader+0x287>
 mov    %eax,(%esp)
...
+jmp    <T> <_ZN17CPacketTranslater25OnDBMWResisterToBlackListEP12PacketHeader+0x280>
 mov    %edx,%ebx
 mov    %eax,%esi
...
+jmp    <T> <_ZN17CPacketTranslater25OnDBMWResisterToBlackListEP12PacketHeader+0x2dc>
 mov    %edx,%ebx
 mov    %eax,%esi
 call   <T> <__cxa_end_catch>
 mov    %esi,%eax
...

{
  ...
  local_14 = param_1;
  if (m_pclApp == 0) {
                    /* try { // try from 0807e20f to 0807e3c4 has its CatchHandler @ 0807e3ca */
    CMyFileLog::CMyFileLog(local_38,"OnDBMWResisterToBlackList",0xf5e);
    ...
    if (local_18 == (CUser *)0x0) {
      CMyFileLog::CMyFileLog(local_30,"OnDBMWResisterToBlackList",0xf65);
      CMyFileLog::_ZN10CMyFileLogclEPKcS1_z
...
定义于 [source/DNFServer/GameServer/Guild/DNFPacketTranslater.cpp](source/DNFServer/GameServer/Guild/DNFPacketTranslater.cpp)（约第 2794 行）：
