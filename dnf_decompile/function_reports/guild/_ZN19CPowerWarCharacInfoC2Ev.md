# _ZN19CPowerWarCharacInfoC2Ev

`CPowerWarCharacInfo::CPowerWarCharacInfo()`

| 服务 | 状态 | ORIG 地址 | ORIG 大小 | 重建地址 | 重建大小 |
|---|---|---|---|---|---|
| guild | DIFF | `0x80a763e` | `0x7a` | `0x809db0a` | `0xaa` |

## 1. 汇编 diff（完整函数，伪代码化）

归一化口径：直接跳转/调用目标地址归一化为 `<T>`；字符串/全局变量地址替换为其内容或 `&符号名`（地址不同但指向相同内容视为等价，2026-08-11 用户口径）。

```diff
--- ORIG（伪代码化）
+++ OURS（伪代码化）
@@ -1,44 +1,58 @@
 push   %ebp
 mov    %esp,%ebp
+push   %edi
 push   %esi
 push   %ebx
-sub    $0x10,%esp
+sub    $0x2c,%esp
 mov    0x8(%ebp),%eax
 movl   $&_ZTV19CPowerWarCharacInfo+0x8,(%eax)
 mov    0x8(%ebp),%eax
-add    $0x4,%eax
+lea    0x4(%eax),%esi
+mov    %esi,0x4(%esp)
+movl   $0x18,(%esp)
+call   <T> <_ZnwjPv>
+mov    %eax,%ebx
+mov    %ebx,%eax
+test   %eax,%eax
+je     <T> <_ZN19CPowerWarCharacInfoC1Ev+0x5a>
+mov    %ebx,%eax
 mov    %eax,(%esp)
 call   <T> <_ZNSt3mapIjP20STPowerWarCharacInfoSt4lessIjESaISt4pairIKjS1_EEEC1Ev>
+jmp    <T> <_ZN19CPowerWarCharacInfoC1Ev+0x5a>
+mov    %edx,%edi
+mov    %eax,-0x20(%ebp)
+mov    %esi,0x4(%esp)
+mov    %ebx,(%esp)
+call   <T> <_ZdlPvS_>
+mov    -0x20(%ebp),%eax
+mov    %edi,%edx
+mov    %eax,(%esp)
+call   <T> <_Unwind_Resume>
 mov    0x8(%ebp),%eax
-add    $0x1c,%eax
+lea    0x1c(%eax),%esi
+mov    %esi,0x4(%esp)
+movl   $0xc,(%esp)
+call   <T> <_ZnwjPv>
+mov    %eax,%ebx
+mov    %ebx,%eax
+test   %eax,%eax
+je     <T> <_ZN19CPowerWarCharacInfoC1Ev+0xa2>
+mov    %ebx,%eax
 mov    %eax,(%esp)
 call   <T> <_ZNSt6vectorIP20STPowerWarCharacInfoSaIS1_EEC1Ev>
-mov    0x8(%ebp),%eax
-add    $0x28,%eax
-mov    %eax,(%esp)
-call   <T> <_ZNSt4listI11STUserPointSaIS0_EEC1Ev>
-jmp    <T> <_ZN19CPowerWarCharacInfoC1Ev+0x73>
-mov    %edx,%ebx
-mov    %eax,%esi
-mov    0x8(%ebp),%eax
-add    $0x1c,%eax
-mov    %eax,(%esp)
-call   <T> <_ZNSt6vectorIP20STPowerWarCharacInfoSaIS1_EED1Ev>
-mov    %esi,%eax
-mov    %ebx,%edx
-jmp    <T> <_ZN19CPowerWarCharacInfoC1Ev+0x55>
-mov    %edx,%ebx
-mov    %eax,%esi
-mov    0x8(%ebp),%eax
-add    $0x4,%eax
-mov    %eax,(%esp)
-call   <T> <_ZNSt3mapIjP20STPowerWarCharacInfoSt4lessIjESaISt4pairIKjS1_EEED1Ev>
-mov    %esi,%eax
-mov    %ebx,%edx
+jmp    <T> <_ZN19CPowerWarCharacInfoC1Ev+0xa2>
+mov    %edx,%edi
+mov    %eax,-0x1c(%ebp)
+mov    %esi,0x4(%esp)
+mov    %ebx,(%esp)
+call   <T> <_ZdlPvS_>
+mov    -0x1c(%ebp),%eax
+mov    %edi,%edx
 mov    %eax,(%esp)
 call   <T> <_Unwind_Resume>
-add    $0x10,%esp
+add    $0x2c,%esp
 pop    %ebx
 pop    %esi
+pop    %edi
 pop    %ebp
 ret
```
## 2. Ghidra 反编译 C

```c

/* CPowerWarCharacInfo::CPowerWarCharacInfo() */

void __thiscall CPowerWarCharacInfo::_ZN19CPowerWarCharacInfoC2Ev(CPowerWarCharacInfo *this)

{
  *(undefined ***)this = &PTR__CPowerWarCharacInfo_08137df0;
  std::
  map<unsigned_int,STPowerWarCharacInfo*,std::less<unsigned_int>,std::allocator<std::pair<unsigned_int_const,STPowerWarCharacInfo*>>>
  ::map((map<unsigned_int,STPowerWarCharacInfo*,std::less<unsigned_int>,std::allocator<std::pair<unsigned_int_const,STPowerWarCharacInfo*>>>
         *)(this + 4));
                    /* try { // try from 080a7666 to 080a766a has its CatchHandler @ 080a7693 */
  std::vector<STPowerWarCharacInfo*,std::allocator<STPowerWarCharacInfo*>>::vector
            ((vector<STPowerWarCharacInfo*,std::allocator<STPowerWarCharacInfo*>> *)(this + 0x1c));
                    /* try { // try from 080a7674 to 080a7678 has its CatchHandler @ 080a767b */
  std::list<STUserPoint,std::allocator<STUserPoint>>::list
            ((list<STUserPoint,std::allocator<STUserPoint>> *)(this + 0x28));
  return;
}
```

## 3. 我们的源码函数

定义于 [source/DNFServer/GameServer/Guild/PowerWarCharacInfo.cpp](source/DNFServer/GameServer/Guild/PowerWarCharacInfo.cpp)（约第 103 行）：

```cpp
CPowerWarCharacInfo::CPowerWarCharacInfo()
{
    new (m_data + 0) std::map<unsigned int, STPowerWarCharacInfo*>();       // class +4
    new (m_data + 0x18) std::vector<STPowerWarCharacInfo*>();               // class +0x1c
}
```
