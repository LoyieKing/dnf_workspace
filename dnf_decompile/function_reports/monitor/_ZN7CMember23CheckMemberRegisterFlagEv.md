...
|---|---|---|---|---|---|
| monitor | DIFF | `0x80991ec` | `0x253` | `0x805fcb6` | `0x202` |
...
+mov    0x8(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN7CMember26CheckDailyScheduleTimeOverEil>
+test   %al,%al
+setne  %al
+mov    %al,-0x51(%ebp)
+movzbl -0x51(%ebp),%eax
...
+setne  %al
+test   %al,%al
+je     <T> <_ZN7CMember23CheckMemberRegisterFlagEv+0x9e>
 mov    0x8(%ebp),%eax
 mov    0x1c0(%eax),%eax
-mov    %eax,0x8(%esp)
-movl   $0x6,0x4(%esp)
...
+mov    0x8(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN7CMember28CheckDayHourScheduleTimeOverEiil>
+test   %al,%al
+setne  %al
+mov    %al,-0x51(%ebp)
+movzbl -0x51(%ebp),%eax
...
+sete   %al
+test   %al,%al
+je     <T> <_ZN7CMember23CheckMemberRegisterFlagEv+0x1f7>
 mov    0x8(%ebp),%eax
 add    $0x1bc,%eax
 mov    %eax,(%esp)
 call   <T> <localtime>
...
+mov    %eax,-0x1c(%ebp)
+movl   $0x336,0x8(%esp)
+movl   $"CheckMemberRegisterFlag",0x4(%esp)
+lea    -0x5c(%ebp),%eax
+mov    %eax,(%esp)
...
定义于 [source/DNFServer/GameServer/Monitor/DNFMember.cpp](source/DNFServer/GameServer/Monitor/DNFMember.cpp)（约第 164 行）：
