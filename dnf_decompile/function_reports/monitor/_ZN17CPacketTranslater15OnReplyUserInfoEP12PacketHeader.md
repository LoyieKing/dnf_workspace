# _ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader

`CPacketTranslater::OnReplyUserInfo(PacketHeader*)`

| 服务 | 状态 | ORIG 地址 | ORIG 大小 | 重建地址 | 重建大小 |
|---|---|---|---|---|---|
| monitor | DIFF | `0x807efb2` | `0x5ef` | `0x806a546` | `0x5fb` |

## 1. 汇编 diff（完整函数，伪代码化）

归一化口径：直接跳转/调用目标地址归一化为 `<T>`；字符串/全局变量地址替换为其内容或 `&符号名`（地址不同但指向相同内容视为等价，2026-08-11 用户口径）。

```diff
--- ORIG（伪代码化）
+++ OURS（伪代码化）
@@ -1,431 +1,434 @@
 push   %ebp
 mov    %esp,%ebp
 push   %edi
 push   %esi
 push   %ebx
-sub    $0x7c,%esp
+sub    $0x8c,%esp
 mov    0x8(%ebp),%eax
-mov    %eax,-0x30(%ebp)
-mov    -0x30(%ebp),%eax
 movzbl 0xb(%eax),%eax
 movzbl %al,%ebx
 movl   $0x361,0x8(%esp)
 movl   $&_ZZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeaderE12__FUNCTION__,0x4(%esp)
-lea    -0x68(%ebp),%eax
+lea    -0x6c(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZN10CMyFileLogC1EPKci>
 mov    %ebx,0xc(%esp)
 movl   $"[GAME SERVER] Channel No : %d\n",0x8(%esp)
 movl   $"./log/Reboot",0x4(%esp)
-lea    -0x68(%ebp),%eax
+lea    -0x6c(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZN10CMyFileLogclEPKcS1_z>
 mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
 test   %eax,%eax
-je     <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x3fa>
-mov    -0x30(%ebp),%eax
-movzbl 0xb(%eax),%eax
-movzbl %al,%edx
-mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
-mov    %edx,0x4(%esp)
-mov    %eax,(%esp)
-call   <T> <_ZN12CApplication14FindGameServerEi>
-mov    %eax,-0x38(%ebp)
-cmpl   $0x0,-0x38(%ebp)
-sete   %al
-test   %al,%al
-je     <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x166>
-lea    -0x59(%ebp),%eax
+jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x136>
+lea    -0x5d(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZNSaIcEC1Ev>
-lea    -0x59(%ebp),%eax
+lea    -0x5d(%ebp),%eax
 mov    %eax,0x8(%esp)
-movl   $"CPacketTranslater::OnReplyUserInfo : pclGameServer == 0",0x4(%esp)
-lea    -0x60(%ebp),%eax
+movl   $"CPacketTranslater::OnReplyUserInfo : m_pclApp == 0",0x4(%esp)
+lea    -0x64(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZNSsC1EPKcRKSaIcE>
-lea    -0x60(%ebp),%esi
+lea    -0x64(%ebp),%esi
 movl   $0x8,(%esp)
 call   <T> <__cxa_allocate_exception>
 mov    %eax,%ebx
 mov    %ebx,%eax
 mov    %esi,0x4(%esp)
 mov    %eax,(%esp)
 call   <T> <_ZN13CDNFExceptionC1ERKSs>
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x10c>
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0xdc>
 mov    %eax,%ecx
 mov    %edx,%eax
 mov    %eax,%esi
 mov    %ecx,%edi
 mov    %ebx,(%esp)
 call   <T> <__cxa_free_exception>
 mov    %edi,%ecx
 mov    %esi,%eax
 mov    %eax,%ebx
 mov    %ecx,%esi
-lea    -0x60(%ebp),%eax
+lea    -0x64(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZNSsD1Ev>
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x106>
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0xd6>
 mov    %eax,%ecx
 mov    %edx,%eax
 cmp    $0xffffffff,%eax
-jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x12b>
+jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0xfb>
 call   <T> <_ZSt9terminatev>
 mov    %esi,%ecx
 mov    %ebx,%eax
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x12b>
-lea    -0x60(%ebp),%eax
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0xfb>
+lea    -0x64(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZNSsD1Ev>
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x143>
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x113>
 mov    %eax,%ecx
 mov    %edx,%eax
 cmp    $0xffffffff,%eax
-jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x12b>
+jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0xfb>
 call   <T> <_ZSt9terminatev>
 mov    %eax,%ecx
 mov    %edx,%eax
 mov    %eax,%ebx
 mov    %ecx,%esi
-lea    -0x59(%ebp),%eax
+lea    -0x5d(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZNSaIcED1Ev>
 mov    %esi,%ecx
 mov    %ebx,%eax
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x4d4>
-lea    -0x59(%ebp),%eax
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x4e0>
+lea    -0x5d(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZNSaIcED1Ev>
 movl   $&_ZN13CDNFExceptionD1Ev,0x8(%esp)
 movl   $&_ZTI13CDNFException,0x4(%esp)
 mov    %ebx,(%esp)
 call   <T> <__cxa_throw>
-mov    -0x30(%ebp),%eax
-mov    0x6(%eax),%edx
+mov    0x8(%ebp),%eax
+movzbl 0xb(%eax),%eax
+movzbl %al,%edx
 mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
 mov    %edx,0x4(%esp)
 mov    %eax,(%esp)
-call   <T> <_ZN12CApplication17FindTcpGameServerEj>
-mov    %eax,-0x34(%ebp)
-cmpl   $0x0,-0x34(%ebp)
-sete   %al
-test   %al,%al
-jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x5e6>
-mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
-add    $0x10,%eax
-mov    %eax,-0x2c(%ebp)
-movl   $0x0,-0x24(%ebp)
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x3dd>
-mov    -0x24(%ebp),%edx
-mov    -0x30(%ebp),%eax
-imul   $0x4e,%edx,%edx
-mov    0xc(%edx,%eax,1),%eax
-test   %eax,%eax
-je     <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x3d8>
-mov    -0x24(%ebp),%edx
-mov    -0x30(%ebp),%eax
-imul   $0x4e,%edx,%edx
-mov    0xc(%edx,%eax,1),%eax
-mov    %eax,0x4(%esp)
-mov    -0x2c(%ebp),%eax
-mov    %eax,(%esp)
-call   <T> <_ZNK12CUserManager8FindUserEj>
-mov    %eax,-0x28(%ebp)
-cmpl   $0x0,-0x28(%ebp)
-setne  %al
-test   %al,%al
-je     <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x243>
-mov    -0x24(%ebp),%edx
-mov    -0x30(%ebp),%eax
-imul   $0x4e,%edx,%edx
-mov    0xc(%edx,%eax,1),%eax
-movl   $0x0,0x4(%esp)
-mov    %eax,(%esp)
-call   <T> <_Z14NumberToStringji>
-mov    %eax,%ebx
-movl   $0x37a,0x8(%esp)
-movl   $&_ZZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeaderE12__FUNCTION__,0x4(%esp)
-lea    -0x58(%ebp),%eax
-mov    %eax,(%esp)
-call   <T> <_ZN10CMyFileLogC1EPKci>
-mov    %ebx,0xc(%esp)
-movl   $"CPacketTranslater::OnReplyUserInfo() : %s\n",0x8(%esp)
-movl   $"./log/Except",0x4(%esp)
-lea    -0x58(%ebp),%eax
-mov    %eax,(%esp)
-call   <T> <_ZN10CMyFileLogclEPKcS1_z>
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x3d9>
-mov    -0x24(%ebp),%eax
-imul   $0x4e,%eax,%eax
-add    -0x30(%ebp),%eax
-add    $0xc,%eax
-mov    %eax,-0x20(%ebp)
-mov    -0x20(%ebp),%eax
-mov    0x8(%eax),%ecx
-mov    -0x20(%ebp),%eax
-lea    0x14(%eax),%esi
-mov    -0x20(%ebp),%eax
-mov    0x4(%eax),%edx
-mov    -0x20(%ebp),%eax
-mov    (%eax),%eax
-mov    -0x38(%ebp),%ebx
-mov    %ebx,0x14(%esp)
-mov    %ecx,0x10(%esp)
-mov    %esi,0xc(%esp)
-mov    %edx,0x8(%esp)
-mov    %eax,0x4(%esp)
-mov    -0x2c(%ebp),%eax
-mov    %eax,(%esp)
-call   <T> <_ZN12CUserManager10CreateUserEjjPciP11CGameServer>
-mov    %eax,-0x28(%ebp)
-mov    -0x20(%ebp),%eax
-lea    0x14(%eax),%esi
-mov    -0x20(%ebp),%eax
-mov    0x4(%eax),%ebx
-mov    -0x20(%ebp),%eax
-movzwl 0x12(%eax),%eax
-movswl %ax,%ecx
-mov    -0x20(%ebp),%eax
-movzbl 0x11(%eax),%eax
-movsbl %al,%edx
-mov    -0x20(%ebp),%eax
-movzbl 0x10(%eax),%eax
-movsbl %al,%eax
-mov    %esi,0x14(%esp)
-mov    %ebx,0x10(%esp)
-mov    %ecx,0xc(%esp)
-mov    %edx,0x8(%esp)
-mov    %eax,0x4(%esp)
-mov    -0x28(%ebp),%eax
-mov    %eax,(%esp)
-call   <T> <_ZN5CUser18SetUserInfo_CharNoEccsjPc>
-mov    -0x20(%ebp),%eax
-movzbl 0x46(%eax),%eax
-movzbl %al,%eax
-mov    %eax,0x4(%esp)
-mov    -0x28(%ebp),%eax
-mov    %eax,(%esp)
-call   <T> <_ZN5CUser6SetSexEh>
-mov    -0x20(%ebp),%eax
-add    $0x47,%eax
-mov    %eax,0x4(%esp)
-mov    -0x28(%ebp),%eax
-mov    %eax,(%esp)
-call   <T> <_ZN5CUser6SetSsnEPc>
-mov    -0x34(%ebp),%eax
-mov    %eax,0x4(%esp)
-mov    -0x28(%ebp),%eax
-mov    %eax,(%esp)
-call   <T> <_ZN5CUser16SetTcpGameServerEP14CTcpGameServer>
-mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
-mov    %eax,(%esp)
-call   <T> <_ZN12CApplication21Get_MemoryCashManagerEv>
-mov    -0x28(%ebp),%edx
-mov    %edx,0x4(%esp)
-mov    %eax,(%esp)
-call   <T> <_ZN18CMemoryCashManager24QueryCashMemoryBlackListEP5CUser>
-xor    $0x1,%eax
-test   %al,%al
-je     <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x347>
-mov    -0x20(%ebp),%eax
-mov    (%eax),%eax
-mov    %eax,(%esp)
-call   <T> <_ZN17CPacketTranslater22RequestBlackListToDBMWEj>
-mov    -0x20(%ebp),%eax
-mov    0x32(%eax),%eax
-test   %eax,%eax
-je     <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x399>
-mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
-mov    %eax,(%esp)
-call   <T> <_ZN12CApplication21Get_MemoryCashManagerEv>
-mov    -0x28(%ebp),%edx
-mov    %edx,0x4(%esp)
-mov    %eax,(%esp)
-call   <T> <_ZN18CMemoryCashManager21QueryCashMemoryMemberEP5CUser>
-xor    $0x1,%eax
-test   %al,%al
-je     <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x399>
-mov    -0x20(%ebp),%eax
-mov    0x32(%eax),%eax
-mov    &_ZN17CPacketTranslater8m_pclAppE,%edx
-lea    0x2d0(%edx),%ecx
-mov    -0x28(%ebp),%edx
-mov    %edx,0x8(%esp)
-mov    %eax,0x4(%esp)
-mov    %ecx,(%esp)
-call   <T> <_ZN14CMemberManager13MemerMemLoginEjP5CUser>
-mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
-mov    %eax,(%esp)
-call   <T> <_ZN12CApplication21Get_MemoryCashManagerEv>
-mov    -0x28(%ebp),%edx
-mov    %edx,0x4(%esp)
-mov    %eax,(%esp)
-call   <T> <_ZN18CMemoryCashManager24QueryCashMemoryBuddyInfoEP5CUser>
-xor    $0x1,%eax
-test   %al,%al
-je     <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x3d9>
-mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
-mov    0xa0(%eax),%eax
-mov    %eax,0x4(%esp)
-mov    -0x28(%ebp),%eax
-mov    %eax,(%esp)
-call   <T> <_ZN5CUser14QueryBuddyInfoEP14CServerHandler>
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x3d9>
-nop
-addl   $0x1,-0x24(%ebp)
-mov    -0x30(%ebp),%eax
-movzbl 0xa(%eax),%eax
-movzbl %al,%eax
-cmp    -0x24(%ebp),%eax
-setg   %al
-test   %al,%al
-jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x1a6>
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x5e7>
-lea    -0x49(%ebp),%eax
+call   <T> <_ZN12CApplication14FindGameServerEi>
+mov    %eax,-0x44(%ebp)
+cmpl   $0x0,-0x44(%ebp)
+jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x237>
+lea    -0x55(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZNSaIcEC1Ev>
-lea    -0x49(%ebp),%eax
+lea    -0x55(%ebp),%eax
 mov    %eax,0x8(%esp)
-movl   $"CPacketTranslater::OnReplyUserInfo : m_pclApp == 0",0x4(%esp)
-lea    -0x50(%ebp),%eax
+movl   $"CPacketTranslater::OnReplyUserInfo : pclGameServer == 0",0x4(%esp)
+lea    -0x5c(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZNSsC1EPKcRKSaIcE>
-lea    -0x50(%ebp),%esi
+lea    -0x5c(%ebp),%esi
 movl   $0x8,(%esp)
 call   <T> <__cxa_allocate_exception>
 mov    %eax,%ebx
 mov    %ebx,%eax
 mov    %esi,0x4(%esp)
 mov    %eax,(%esp)
 call   <T> <_ZN13CDNFExceptionC1ERKSs>
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x479>
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x1dd>
 mov    %eax,%ecx
 mov    %edx,%eax
 mov    %eax,%esi
 mov    %ecx,%edi
 mov    %ebx,(%esp)
 call   <T> <__cxa_free_exception>
 mov    %edi,%ecx
 mov    %esi,%eax
 mov    %eax,%ebx
 mov    %ecx,%esi
-lea    -0x50(%ebp),%eax
+lea    -0x5c(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZNSsD1Ev>
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x473>
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x1d7>
 mov    %eax,%ecx
 mov    %edx,%eax
 cmp    $0xffffffff,%eax
-jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x498>
+jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x1fc>
 call   <T> <_ZSt9terminatev>
 mov    %esi,%ecx
 mov    %ebx,%eax
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x498>
-lea    -0x50(%ebp),%eax
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x1fc>
+lea    -0x5c(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZNSsD1Ev>
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x4ad>
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x214>
 mov    %eax,%ecx
 mov    %edx,%eax
 cmp    $0xffffffff,%eax
-jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x498>
+jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x1fc>
 call   <T> <_ZSt9terminatev>
 mov    %eax,%ecx
 mov    %edx,%eax
 mov    %eax,%ebx
 mov    %ecx,%esi
-lea    -0x49(%ebp),%eax
+lea    -0x55(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZNSaIcED1Ev>
 mov    %esi,%ecx
 mov    %ebx,%eax
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x4d4>
-lea    -0x49(%ebp),%eax
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x4e0>
+lea    -0x55(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZNSaIcED1Ev>
 movl   $&_ZN13CDNFExceptionD1Ev,0x8(%esp)
 movl   $&_ZTI13CDNFException,0x4(%esp)
 mov    %ebx,(%esp)
 call   <T> <__cxa_throw>
+mov    0x8(%ebp),%eax
+movzbl 0xb(%eax),%eax
+movzbl %al,%edx
+mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
+mov    %edx,0x4(%esp)
+mov    %eax,(%esp)
+call   <T> <_ZN12CApplication17FindTcpGameServerEj>
+mov    %eax,-0x40(%ebp)
+cmpl   $0x0,-0x40(%ebp)
+je     <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x5f0>
+mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
+add    $0x10,%eax
+mov    %eax,-0x3c(%ebp)
+movl   $0x0,-0x38(%ebp)
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x4bf>
+mov    0x8(%ebp),%eax
+mov    -0x38(%ebp),%edx
+imul   $0x4e,%edx,%edx
+add    $0xc,%edx
+add    %edx,%eax
+mov    %eax,-0x34(%ebp)
+mov    -0x34(%ebp),%eax
+mov    (%eax),%eax
+test   %eax,%eax
+je     <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x4bb>
+mov    -0x34(%ebp),%eax
+mov    (%eax),%eax
+mov    %eax,0x4(%esp)
+mov    -0x3c(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZNK12CUserManager8FindUserEj>
+mov    %eax,-0x30(%ebp)
+cmpl   $0x0,-0x30(%ebp)
+jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x466>
+mov    -0x34(%ebp),%eax
+add    $0x8,%eax
+mov    (%eax),%ecx
+mov    -0x34(%ebp),%eax
+lea    0x14(%eax),%esi
+mov    -0x34(%ebp),%eax
+add    $0x4,%eax
+mov    (%eax),%edx
+mov    -0x34(%ebp),%eax
+mov    (%eax),%eax
+mov    -0x44(%ebp),%ebx
+mov    %ebx,0x14(%esp)
+mov    %ecx,0x10(%esp)
+mov    %esi,0xc(%esp)
+mov    %edx,0x8(%esp)
+mov    %eax,0x4(%esp)
+mov    -0x3c(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN12CUserManager10CreateUserEjjPciP11CGameServer>
+mov    %eax,-0x30(%ebp)
+mov    -0x34(%ebp),%eax
+lea    0x14(%eax),%esi
+mov    -0x34(%ebp),%eax
+add    $0x4,%eax
+mov    (%eax),%ebx
+mov    -0x34(%ebp),%eax
+add    $0x12,%eax
+movzwl (%eax),%eax
+movswl %ax,%ecx
+mov    -0x34(%ebp),%eax
+add    $0x11,%eax
+movzbl (%eax),%eax
+movsbl %al,%edx
+mov    -0x34(%ebp),%eax
+add    $0x10,%eax
+movzbl (%eax),%eax
+movsbl %al,%eax
+mov    %esi,0x14(%esp)
+mov    %ebx,0x10(%esp)
+mov    %ecx,0xc(%esp)
+mov    %edx,0x8(%esp)
+mov    %eax,0x4(%esp)
+mov    -0x30(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN5CUser18SetUserInfo_CharNoEccsjPc>
+mov    -0x34(%ebp),%eax
+add    $0x46,%eax
+movzbl (%eax),%eax
+movzbl %al,%eax
+mov    %eax,0x4(%esp)
+mov    -0x30(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN5CUser6SetSexEh>
+mov    -0x34(%ebp),%eax
+add    $0x47,%eax
+mov    %eax,0x4(%esp)
+mov    -0x30(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN5CUser6SetSsnEPc>
+mov    -0x40(%ebp),%eax
+mov    %eax,0x4(%esp)
+mov    -0x30(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN5CUser16SetTcpGameServerEP14CTcpGameServer>
+mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
+mov    %eax,(%esp)
+call   <T> <_ZN12CApplication21Get_MemoryCashManagerEv>
+mov    %eax,-0x2c(%ebp)
+mov    -0x30(%ebp),%eax
+mov    %eax,0x4(%esp)
+mov    -0x2c(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN18CMemoryCashManager24QueryCashMemoryBlackListEP5CUser>
+cmp    $0x1,%al
+setne  %al
+test   %al,%al
+je     <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x3c0>
+mov    -0x34(%ebp),%eax
+mov    (%eax),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN17CPacketTranslater22RequestBlackListToDBMWEj>
+mov    -0x34(%ebp),%eax
+add    $0x32,%eax
+mov    (%eax),%eax
+test   %eax,%eax
+je     <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x41e>
+mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
+mov    %eax,(%esp)
+call   <T> <_ZN12CApplication21Get_MemoryCashManagerEv>
+mov    %eax,-0x24(%ebp)
+mov    -0x30(%ebp),%eax
+mov    %eax,0x4(%esp)
+mov    -0x24(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN18CMemoryCashManager21QueryCashMemoryMemberEP5CUser>
+cmp    $0x1,%al
+setne  %al
+test   %al,%al
+je     <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x41e>
+mov    -0x34(%ebp),%eax
+add    $0x32,%eax
+mov    (%eax),%eax
+mov    &_ZN17CPacketTranslater8m_pclAppE,%edx
+lea    0x2d0(%edx),%ecx
+mov    -0x30(%ebp),%edx
+mov    %edx,0x8(%esp)
+mov    %eax,0x4(%esp)
+mov    %ecx,(%esp)
+call   <T> <_ZN14CMemberManager13MemerMemLoginEjP5CUser>
+mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
+mov    %eax,(%esp)
+call   <T> <_ZN12CApplication21Get_MemoryCashManagerEv>
+mov    %eax,-0x28(%ebp)
+mov    -0x30(%ebp),%eax
+mov    %eax,0x4(%esp)
+mov    -0x28(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN18CMemoryCashManager24QueryCashMemoryBuddyInfoEP5CUser>
+cmp    $0x1,%eax
+setne  %al
+test   %al,%al
+je     <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x4bb>
+mov    &_ZN17CPacketTranslater8m_pclAppE,%eax
+mov    0xa0(%eax),%eax
+mov    %eax,0x4(%esp)
+mov    -0x30(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN5CUser14QueryBuddyInfoEP14CServerHandler>
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x4bb>
+mov    -0x34(%ebp),%eax
+mov    (%eax),%eax
+movl   $0x0,0x4(%esp)
+mov    %eax,(%esp)
+call   <T> <_Z14NumberToStringji>
+mov    %eax,-0x20(%ebp)
+movl   $0x37a,0x8(%esp)
+movl   $"OnReplyUserInfo",0x4(%esp)
+lea    -0x74(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN10CMyFileLogC1EPKci>
+mov    -0x20(%ebp),%eax
+mov    %eax,0xc(%esp)
+movl   $"CPacketTranslater::OnReplyUserInfo() : %s\n",0x8(%esp)
+movl   $"./log/Except",0x4(%esp)
+lea    -0x74(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN10CMyFileLogclEPKcS1_z>
+addl   $0x1,-0x38(%ebp)
+mov    0x8(%ebp),%eax
+movzbl 0xa(%eax),%eax
+movzbl %al,%eax
+cmp    -0x38(%ebp),%eax
+setg   %al
+test   %al,%al
+jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x276>
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x5f0>
 mov    %eax,%ecx
 mov    %edx,%eax
 cmp    $0x2,%eax
-jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x57a>
+jne    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x586>
 mov    %ecx,(%esp)
 call   <T> <__cxa_begin_catch>
 mov    %eax,-0x1c(%ebp)
 mov    -0x1c(%ebp),%eax
 mov    (%eax),%eax
 add    $0x8,%eax
 mov    (%eax),%edx
 mov    -0x1c(%ebp),%eax
 mov    %eax,(%esp)
 call   *%edx
 mov    %eax,0x4(%esp)
 movl   $"CPacketTranslater::OnReplyUserInfo() Exception Break : %s\n",(%esp)
 call   <T> <printf>
 mov    -0x1c(%ebp),%eax
 mov    (%eax),%eax
 add    $0x8,%eax
 mov    (%eax),%edx
 mov    -0x1c(%ebp),%eax
 mov    %eax,(%esp)
 call   *%edx
 mov    %eax,%ebx
 movl   $0x3ac,0x8(%esp)
 movl   $&_ZZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeaderE12__FUNCTION__,0x4(%esp)
-lea    -0x48(%ebp),%eax
+lea    -0x54(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZN10CMyFileLogC1EPKci>
 mov    %ebx,0xc(%esp)
 movl   $"CPacketTranslater::OnReplyUserInfo() Exception Break : %s\n",0x8(%esp)
 movl   $"./log/Except",0x4(%esp)
-lea    -0x48(%ebp),%eax
+lea    -0x54(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZN10CMyFileLogclEPKcS1_z>
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x573>
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x57f>
 mov    %eax,%ecx
 mov    %edx,%eax
 mov    %eax,%ebx
 mov    %ecx,%esi
 call   <T> <__cxa_end_catch>
 mov    %esi,%ecx
 mov    %ebx,%eax
 mov    %ecx,(%esp)
 call   <T> <_Unwind_Resume>
 call   <T> <__cxa_end_catch>
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x5e7>
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x5f0>
 mov    %ecx,(%esp)
 call   <T> <__cxa_begin_catch>
 movl   $"CPacketTranslater::OnReplyUserInfo() Exception Break",(%esp)
 call   <T> <puts>
 movl   $0x3b2,0x8(%esp)
 movl   $&_ZZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeaderE12__FUNCTION__,0x4(%esp)
-lea    -0x40(%ebp),%eax
+lea    -0x4c(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZN10CMyFileLogC1EPKci>
 movl   $"CPacketTranslater::OnReplyUserInfo() Exception Break\n",0x8(%esp)
 movl   $"./log/Except",0x4(%esp)
-lea    -0x40(%ebp),%eax
+lea    -0x4c(%ebp),%eax
 mov    %eax,(%esp)
 call   <T> <_ZN10CMyFileLogclEPKcS1_z>
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x5df>
+jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x5eb>
 mov    %eax,%ecx
 mov    %edx,%eax
 mov    %eax,%ebx
 mov    %ecx,%esi
 call   <T> <__cxa_end_catch>
 mov    %esi,%ecx
 mov    %ebx,%eax
 mov    %ecx,(%esp)
 call   <T> <_Unwind_Resume>
 call   <T> <__cxa_end_catch>
-jmp    <T> <_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader+0x5e7>
-nop
-add    $0x7c,%esp
+add    $0x8c,%esp
 pop    %ebx
 pop    %esi
 pop    %edi
 pop    %ebp
 ret
```
## 2. Ghidra 反编译 C

```c

/* CPacketTranslater::OnReplyUserInfo(PacketHeader*) */

void CPacketTranslater::_ZN17CPacketTranslater15OnReplyUserInfoEP12PacketHeader
               (PacketHeader *param_1)

{
  PacketHeader PVar1;
  char cVar2;
  CDNFException *pCVar3;
  undefined4 uVar4;
  CMemoryCashManager *pCVar5;
  CMyFileLog local_6c [8];
  string local_64 [7];
  allocator local_5d;
  CMyFileLog local_5c [8];
  string local_54 [7];
  allocator local_4d [17];
  CGameServer *local_3c;
  CTcpGameServer *local_38;
  PacketHeader *local_34;
  CUserManager *local_30;
  CUser *local_2c;
  int local_28;
  PacketHeader *local_24;
  
  local_34 = param_1;
  PVar1 = param_1[0xb];
                    /* try { // try from 0807efe1 to 0807f02c has its CatchHandler @ 0807f482 */
  CMyFileLog::CMyFileLog(local_6c,"OnReplyUserInfo",0x361);
  CMyFileLog::_ZN10CMyFileLogclEPKcS1_z
            (local_6c,"./log/Reboot","[GAME SERVER] Channel No : %d\n",(uint)(byte)PVar1);
  if (m_pclApp == (CApplication *)0x0) {
    std::allocator<char>::allocator();
                    /* try { // try from 0807f3cc to 0807f3d0 has its CatchHandler @ 0807f446 */
    std::string::string(local_54,"CPacketTranslater::OnReplyUserInfo : m_pclApp == 0",local_4d);
    pCVar3 = (CDNFException *)__cxa_allocate_exception(8);
                    /* try { // try from 0807f3eb to 0807f3ef has its CatchHandler @ 0807f3f2 */
    CDNFException::CDNFException(pCVar3,local_54);
                    /* try { // try from 0807f431 to 0807f435 has its CatchHandler @ 0807f438 */
    std::string::~string(local_54);
    std::allocator<char>::~allocator((allocator<char> *)local_4d);
                    /* WARNING: Subroutine does not return */
                    /* try { // try from 0807f47d to 0807f481 has its CatchHandler @ 0807f482 */
    __cxa_throw(pCVar3,&CDNFException::typeinfo,CDNFException::_ZN13CDNFExceptionD2Ev);
  }
  local_3c = (CGameServer *)CApplication::FindGameServer((int)m_pclApp);
  if (local_3c != (CGameServer *)0x0) {
    local_38 = (CTcpGameServer *)CApplication::FindTcpGameServer((uint)m_pclApp);
    if (local_38 != (CTcpGameServer *)0x0) {
      local_30 = (CUserManager *)(m_pclApp + 0x10);
      for (local_28 = 0; local_28 < (int)(uint)(byte)local_34[10]; local_28 = local_28 + 1) {
        if (*(int *)(local_34 + local_28 * 0x4e + 0xc) != 0) {
          local_2c = (CUser *)CUserManager::FindUser((uint)local_30);
          if (local_2c == (CUser *)0x0) {
            local_24 = local_34 + local_28 * 0x4e + 0xc;
            local_2c = (CUser *)CUserManager::_ZN12CUserManager10CreateUserEjjPciP11CGameServer
                                          (local_30,*(uint *)local_24,
                                           *(uint *)(local_34 + local_28 * 0x4e + 0x10),
                                           (char *)(local_34 + local_28 * 0x4e + 0x20),
                                           *(int *)(local_34 + local_28 * 0x4e + 0x14),local_3c);
            CUser::SetUserInfo_CharNo
                      (local_2c,(char)local_24[0x10],(char)local_24[0x11],
                       *(short *)(local_24 + 0x12),*(uint *)(local_24 + 4),(char *)(local_24 + 0x14)
                      );
            CUser::SetSex(local_2c,(uchar)local_24[0x46]);
            CUser::SetSsn(local_2c,(char *)(local_24 + 0x47));
            CUser::SetTcpGameServer(local_2c,local_38);
            pCVar5 = (CMemoryCashManager *)
                     CApplication::_ZN12CApplication21Get_MemoryCashManagerEv(m_pclApp);
            cVar2 = CMemoryCashManager::QueryCashMemoryBlackList(pCVar5,local_2c);
            if (cVar2 != '\x01') {
              RequestBlackListToDBMW(*(uint *)local_24);
            }
            if (*(int *)(local_24 + 0x32) != 0) {
              pCVar5 = (CMemoryCashManager *)
                       CApplication::_ZN12CApplication21Get_MemoryCashManagerEv(m_pclApp);
              cVar2 = CMemoryCashManager::QueryCashMemoryMember(pCVar5,local_2c);
              if (cVar2 != '\x01') {
                CMemberManager::_ZN14CMemberManager13MemerMemLoginEjP5CUser
                          ((CMemberManager *)(m_pclApp + 0x2d0),*(uint *)(local_24 + 0x32),local_2c)
                ;
              }
            }
            pCVar5 = (CMemoryCashManager *)
                     CApplication::_ZN12CApplication21Get_MemoryCashManagerEv(m_pclApp);
            cVar2 = CMemoryCashManager::QueryCashMemoryBuddyInfo(pCVar5,local_2c);
            if (cVar2 != '\x01') {
              CUser::QueryBuddyInfo(local_2c,*(CServerHandler **)(m_pclApp + 0xa0));
            }
          }
          else {
            uVar4 = NumberToString(*(uint *)(local_34 + local_28 * 0x4e + 0xc),0);
            CMyFileLog::CMyFileLog(local_5c,"OnReplyUserInfo",0x37a);
            CMyFileLog::_ZN10CMyFileLogclEPKcS1_z
                      (local_5c,"./log/Except","CPacketTranslater::OnReplyUserInfo() : %s\n",uVar4);
          }
        }
      }
    }
    return;
  }
  std::allocator<char>::allocator();
                    /* try { // try from 0807f05f to 0807f063 has its CatchHandler @ 0807f0d9 */
  std::string::string(local_64,"CPacketTranslater::OnReplyUserInfo : pclGameServer == 0",&local_5d);
  pCVar3 = (CDNFException *)__cxa_allocate_exception(8);
                    /* try { // try from 0807f07e to 0807f082 has its CatchHandler @ 0807f085 */
  CDNFException::CDNFException(pCVar3,local_64);
                    /* try { // try from 0807f0c4 to 0807f0c8 has its CatchHandler @ 0807f0cb */
  std::string::~string(local_64);
  std::allocator<char>::~allocator((allocator<char> *)&local_5d);
                    /* WARNING: Subroutine does not return */
                    /* try { // try from 0807f113 to 0807f387 has its CatchHandler @ 0807f482 */
  __cxa_throw(pCVar3,&CDNFException::typeinfo,CDNFException::_ZN13CDNFExceptionD2Ev);
}
```

## 3. 我们的源码函数

定义于 [source/DNFServer/GameServer/Monitor/DNFPacketTranslater.cpp](source/DNFServer/GameServer/Monitor/DNFPacketTranslater.cpp)（约第 480 行）：

```cpp
void CPacketTranslater::OnReplyUserInfo(PacketHeader* pkt)
{
    try
    {
        DNF_LOG_SCOPE_LINE(0x361,"./log/Reboot", "[GAME SERVER] Channel No : %d\n",
            (unsigned int)(unsigned char)((RA_S8<11>*)pkt)->v);
        if (m_pclApp == 0)
        {
            throw CDNFException("CPacketTranslater::OnReplyUserInfo : m_pclApp == 0");
        }
        CGameServer* gs =
            (CGameServer*)m_pclApp->FindGameServer(
                (int)(unsigned char)((RA_S8<11>*)pkt)->v);
        if (gs == 0)
        {
            throw CDNFException("CPacketTranslater::OnReplyUserInfo : pclGameServer == 0");
        }
        CTcpGameServer* tcpGs =
            (CTcpGameServer*)m_pclApp->FindTcpGameServer(
                (unsigned int)(unsigned char)((RA_S8<11>*)pkt)->v);
        if (tcpGs != 0)
        {
            CUserManager* userMgr = (CUserManager*)((char*)m_pclApp + 0x10);
            for (int i = 0;
                 i < (int)(unsigned int)(unsigned char)((RA_S8<10>*)pkt)->v; i++)
            {
                char* entry = (char*)pkt + i * 0x4e + 0xc;
                if (*(int*)entry != 0)
                {
                    CUser* user = userMgr->FindUser(*(unsigned int*)entry);
                    if (user == 0)
                    {
                        user = userMgr->CreateUser(
                            *(unsigned int*)entry, *(unsigned int*)(entry + 4),
                            entry + 0x14, *(int*)(entry + 8), gs);
                        user->SetUserInfo_CharNo(
                            *(char*)(entry + 0x10), *(char*)(entry + 0x11),
                            *(short*)(entry + 0x12), *(unsigned int*)(entry + 4),
                            entry + 0x14);
                        user->SetSex(*(unsigned char*)(entry + 0x46));
                        user->SetSsn(entry + 0x47);
                        user->SetTcpGameServer(tcpGs);
                        CMemoryCashManager* cash =
                            (CMemoryCashManager*)m_pclApp->Get_MemoryCashManager();
                        if (cash->QueryCashMemoryBlackList(user) != 1)
                        {
                            RequestBlackListToDBMW(*(unsigned int*)entry);
                        }
                        if (*(int*)(entry + 0x32) != 0)
                        {
                            CMemoryCashManager* cash2 =
                                (CMemoryCashManager*)m_pclApp->Get_MemoryCashManager();
                            if (cash2->QueryCashMemoryMember(user) != 1)
                            {
                                ((CMemberManager*)((char*)m_pclApp + 0x2d0))
                                    ->MemerMemLogin(*(unsigned int*)(entry + 0x32), user);
                            }
                        }
                        CMemoryCashManager* cash3 =
                            (CMemoryCashManager*)m_pclApp->Get_MemoryCashManager();
                        if (cash3->QueryCashMemoryBuddyInfo(user) != 1)
                        {
                            user->QueryBuddyInfo(
                                m_pclApp->m_serverHandler2);
                        }
                    }
                    else
                    {
                        char* dbid = NumberToString(*(unsigned int*)entry, 0);
                        CMyFileLog log2("OnReplyUserInfo", 0x37a);
                        log2("./log/Except", "CPacketTranslater::OnReplyUserInfo() : %s\n", dbid);
                    }
                }
            }
        }
    }
    catch (CDNFException& e)
    {
        printf("CPacketTranslater::OnReplyUserInfo() Exception Break : %s\n", e.what());
        DNF_LOG_SCOPE_LINE(0x3ac, "./log/Except", "CPacketTranslater::OnReplyUserInfo() Exception Break : %s\n", e.what());
    }
    catch (...)
    {
        puts("CPacketTranslater::OnReplyUserInfo() Exception Break");
        DNF_LOG_SCOPE_LINE(0x3b2, "./log/Except", "CPacketTranslater::OnReplyUserInfo() Exception Break\n");
    }
}
```
