...
|---|---|---|---|---|---|
| monitor | NEAR | `0x8071204` | `0xdf` | `0x808c018` | `0xdf` |
...

{
  ...
  if (param_2 != (CUser *)0x0) {
    ...
    uVar1 = param_1;
    CMyFileLog::CMyFileLog(local_24,"InsertUser_CharNo",0x1d0);
...
定义于 [source/DNFServer/GameServer/Monitor/DNFUserManager.cpp](source/DNFServer/GameServer/Monitor/DNFUserManager.cpp)（约第 442 行）：
...
bool CUserManager::InsertUser_CharNo(const unsigned int charNo, CUser* user)
{
    if (user != 0)
    {
        ...
        register unsigned int dbid;
        register char* charName;
        register unsigned int cNo;
        charName = user->GetCharName();
        dbid = user->GetDBID();
        cNo = charNo;
