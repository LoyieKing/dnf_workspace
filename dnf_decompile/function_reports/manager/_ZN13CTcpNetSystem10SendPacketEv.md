...
|---|---|---|---|---|---|
| manager | DIFF | `0x8058bf4` | `0x2f1` | `0x8066dbc` | `0x32a` |
...
+jne    <T> <_ZN13CTcpNetSystem10SendPacketEv+0x315>
 mov    0x8(%ebp),%eax
 add    $0xc0,%eax
 mov    %eax,(%esp)
...
+mov    %eax,-0x18(%ebp)
+cmpl   $0x0,-0x18(%ebp)
+je     <T> <_ZN13CTcpNetSystem10SendPacketEv+0x315>
+mov    -0x18(%ebp),%eax
+add    $0x6,%eax
+mov    (%eax),%eax
+mov    %eax,-0x14(%ebp)
...
+jmp    <T> <_ZN13CTcpNetSystem10SendPacketEv+0x315>
+lea    -0x28(%ebp),%eax
+mov    %eax,(%esp)
...
+mov    %eax,-0x10(%ebp)
+cmpl   $0x0,-0x10(%ebp)
+jne    <T> <_ZN13CTcpNetSystem10SendPacketEv+0x1b3>
+movl   $0xba,0x8(%esp)
+movl   $&_ZZN13CTcpNetSystem10SendPacketEvE12__FUNCTION__,0x4(%esp)
+lea    -0x38(%ebp),%eax
+mov    %eax,(%esp)
...
+sete   %al
+test   %al,%al
+je     <T> <_ZN13CTcpNetSystem10SendPacketEv+0x248>
+movl   $0xc3,0x8(%esp)
+movl   $&_ZZN13CTcpNetSystem10SendPacketEvE12__FUNCTION__,0x4(%esp)
+lea    -0x40(%ebp),%eax
+mov    %eax,(%esp)
...
+mov    -0x10(%ebp),%eax
+mov    %eax,0xc(%esp)
+movl   $"SEND FAIL(peer:%p, id:%d, size:%d, port:%d)",0x8(%esp)
+movl   $"./log/TcpSend",0x4(%esp)
+lea    -0x40(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN10CMyFileLogclEPKcS1_z>
...
+mov    %eax,-0xc(%ebp)
+cmpl   $0x0,-0xc(%ebp)
+jle    <T> <_ZN13CTcpNetSystem10SendPacketEv+0x28a>
+mov    -0x18(%ebp),%eax
+mov    %eax,0x4(%esp)
+mov    0x8(%ebp),%eax
+mov    %eax,(%esp)
...
+mov    %ebx,0x10(%esp)
+mov    %eax,0xc(%esp)
+movl   $"SEND QUEUE(%d, id:%d, size:%d, port:%d)",0x8(%esp)
+movl   $"./log/TcpSend",0x4(%esp)
+lea    -0x48(%ebp),%eax
+mov    %eax,(%esp)
+call   <T> <_ZN10CMyFileLogclEPKcS1_z>
...

{
  ...
  CGuard<CMutex>::CGuard(local_54,(CMutex *)(this + 0xe8));
                    /* try { // try from 08058c1f to 08058c43 has its CatchHandler @ 08058c50 */
...
定义于 [source/DNFServer/GameServer/Manager/TcpNetSystem.cpp](source/DNFServer/GameServer/Manager/TcpNetSystem.cpp)（约第 191 行）：
