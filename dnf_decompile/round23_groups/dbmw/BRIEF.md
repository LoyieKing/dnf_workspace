# dbmw 第 23 轮类粒度回归 —— 子 Agent 手册（2026-08-12）
...
直到完成或穷举合理纯 C++ 变体后记录证据，最后统一上报父 Agent。
...
- ORIG：`/home/loyieking/dnf_workspace/dnf_installer/build/dnf_data/home/template/neople/dbmw_guild/df_dbmw_r`
  （32 位 ELF，无 DWARF）
- 权威重建：`build/dbmw/df_dbmw_r`（c6 4.4.7 驱动 + 逐文件 cc1plus 4.4.6-3，
  见 `source/cmake/dbmw/CMakeLists.txt` 的 SRC_OPTS）
- 基线：dbmw 非 identical md = **238**（DIFF 230 + NEAR 8；
  manifest：`function_reports/dbmw/manifest.tsv`）
...
- 你组函数清单：`round23_groups/dbmw/<group>.tsv`（demangled / mangled / status /
  orig_size / tu 五列）
- 每个函数的 md：`function_reports/dbmw/<mangled>.md`（ORIG/OURS 完整 diff、
  Ghidra 反编译 C、源码定位；部分函数 md 无“定义于”段，用 demangled 名在
  TU 文件里搜索定位）
...
- 若怀疑某 TU 编译器版本不匹配：用
  ...
  （路径用 `grep -h DF_CC1P446 source/toolchain/cmake/*.cmake` 查，或
  `ls /tmp/cc1plus446bin/`），取助记符/寄存器形态最接近者，附证据上报；
...
写 `/home/loyieking/dnf_workspace/dnf_decompile/round23_groups/dbmw/<group>_report.md`：
