...
int CUdpHandler::SendToClient(const char* buf, int len, unsigned int ip,
                              unsigned short port) const
{
    sockaddr to;
    memset(&to, 0, 0x10);
    ...
    *(unsigned int*)(to.sa_data + 2) = htonl(ip);
    return sendto(m_sock, buf, len, 0, &to, 0x10);
}

void CUdpHandler::Close()
{
    if (m_sock != -1)
    {
        close(m_sock);
