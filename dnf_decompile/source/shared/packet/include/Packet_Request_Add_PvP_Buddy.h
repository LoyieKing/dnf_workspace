...
public:
    int m_id;                         // offset 10（Community OnReqAddBuddy：find_user(m_id) 用户查找键
                                      //          = acc_id；df_game_r SendReqAddPvPBuddy param_1 =
                                      //          CUser::get_acc_id；原始日志串 "M_ID(%d)"）
    int charac_no;                    // offset 14（OnReqAddBuddy：user->charac_no == packet->charac_no；
                                      //          df_game_r param_2 = CUserCharacInfo::getCurCharacNo）
    char server_id;                   // offset 0x12（OnReqAddBuddy：find_buddy/find_user(server_id, name)；
                                      //          df_game_r param_3 服务区字节）
    char m_name[0x1e];                // offset 0x13（OnReqAddBuddy：被加好友角色名，CFindUser 按
                                      //          stGameUserInfo.m_name strcmp；df_game_r param_4 字符串，
                                      //          原占位 buddy_n_user_id_what）
    Packet_Request_Add_PvP_Buddy() : PacketHeader(0x1b5c, 0x31),
