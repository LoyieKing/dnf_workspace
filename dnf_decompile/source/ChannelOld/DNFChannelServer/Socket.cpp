...
TCPSocket::~TCPSocket()
{
    // Verified from original: dtor always calls close()
    close();
}

bool TCPSocket::open()
{
    // Match original control flow: socket → fail+printf / else setOptLinger
    sock_ = socket(2, 1, 6);
    if (sock_ == -1)
    {
...
bool TCPSocket::connect(const char* ip, unsigned short port)
{
    ...
    if (::connect(sock_, (sockaddr*)&svradr, len) < 0)
    {
        ...
        ChannelServiceApp::gFileLogError << "[TCPSocket::connect] Can't connect : ip" << ip << "port=" << port << endl;
...
{
    // Verified from original df_channel_r disassembly (88 insn)
    socklen_t len = 0x10;
    ...
    // c_adrs_ ← adrs_.sin_addr (4B); port_ ← adrs_.sin_port
    ...
    accepted.setOptResizeRecvBuf(0x7800);
    return true;
}
...
bool TCPSocket::pollReadEvent() const
{
    fd_set set;
    int result;
    // ORIG：非 GNU 版 FD_ZERO（fd_set* 基址局部 + 无符号计数），不用 asm/memset 内联
    unsigned int __i;
    fd_set* __arr = &set;
...
bool UDPSocket::open()
{
    // Verified from original: reject if already open; printf errno on failure
    if (sock_ != -1)
    {
        return false;
    }
...
int UDPSocket::send(char* buf, int size, unsigned short nPort, const char* szDestIp)
{
    ...
    if (n_bytes < 0)
    {
        if ((*__errno_location() == 0xb) || (*__errno_location() == 0xb) || (*__errno_location() == 4))
        {
            return 0;
        }
    }
...
int UDPSocket::recv(char* buf, int size)
{
    ...
    int len = 0x10;
    int nRead = recvfrom(sock_, buf, size, 0, (sockaddr*)&from_, (socklen_t*)&len);
    if (nRead < 0)
    {
        return 0;
    }
