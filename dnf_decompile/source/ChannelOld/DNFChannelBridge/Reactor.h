...
public:
    ...
    void startup()
    {
        ...
        if (epoll_fd_ == -1)
        {
            throw Exception(-2, "[EpollReactor< TSession >::Init] Can't create epoll device : max_client(%d)\n", max_client_);
        }
        events_ = new epoll_event[max_client_];
        if (events_ == NULL)
        {
            throw Exception(-3, "[EpollReactor< TSession >::Init] Can't allocate epoll repository : max_client(%d)\n", max_client_);
        }
    }
    void shutdown()
    {
    ...
    bool handleEvents(unsigned int milisec);
...
public:
    ...
    bool handleEvents(unsigned int milisec)
    {
        return impl_.handleEvents(milisec);
    }
private:
    TReactorImpl impl_;
};
