...
FLAGS="-m32 -O0 -std=gnu++0x -fno-enforce-eh-specs -nostdinc -DTIXML_USE_STL -DBOOST_DISABLE_ASSERTS \
  ...
  -I$COMMON -I$MANAGER -I$PACKET/include \
...
compile() {
    ...
    if [ ! -f "$OUT_DIR/$base.o" ]; then
        ...
        for h in "$MANAGER"/*.h "$COMMON"/*.h; do
            [ -f "$h" ] || continue
...
if ls "$MANAGER"/*.cpp >/dev/null 2>&1; then
    for f in "$MANAGER"/*.cpp; do
        [ -f "$f" ] && compile "$f"
    done
fi
...
if [ -n "$OBJS" ]; then
    g++ -m32 -no-pie -o "$OUT_DIR/df_manager_r" $OBJS \
        "$OUT_DIR/mysqlclient/trees.o" \
        "$OUT_DIR/mysqlclient/libmysqlclient.a" \
        ...
        -lpthread -ldl -lm
    echo "LINK OK -> $OUT_DIR/df_manager_r"
fi
