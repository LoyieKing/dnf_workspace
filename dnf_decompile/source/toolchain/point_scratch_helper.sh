#!/bin/bash
# point scratch-build helper (round 14, class-granular subagents)
#
# Usage: source this file, then:
#   scratch_setup <group>                 # copy all .o into /tmp/sub_point_<group>
...
#
# Design: the scratch dir mirrors build/point's CMakeFiles/point.dir layout, so
...
BUILD="$REPO/build/point"
CXX="$REPO/source/toolchain/cmake/dnf_c6446r_gxx.sh"
FLAGS=$(grep '^CXX_FLAGS =' "$BUILD/CMakeFiles/point.dir/flags.make" | sed 's/^CXX_FLAGS = //')

scratch_dir() { echo "/tmp/sub_point_$1"; }
...
scratch_setup() {
    ...
    cp -r "$BUILD/CMakeFiles/point.dir" "$scr/CMakeFiles/"
...
scratch_compile() {
    ...
    local scr; scr=$(scratch_dir "$g")
    local rel=${src#/}.o
    local out="$scr/CMakeFiles/point.dir/$rel"
    mkdir -p "$(dirname "$out")"
    echo "CC  $src"
...
scratch_link() {
    local g=$1
    local scr; scr=$(scratch_dir "$g")
    echo "LD  $scr/df_point_r"
    (cd "$scr" && sh "$BUILD/CMakeFiles/point.dir/link.txt") || return 1
}
...
scratch_classify() {
    ...
    (cd "$REPO" && python3 source/toolchain/gen_function_md.py \
        --services point --function "$name" \
        --new-bin "$scr/df_point_r" --check-only)
}
