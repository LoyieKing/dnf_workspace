...
#pragma pack(push, 1)  // 原版数据包头 packed(1)：m_dwSrcIp@6、m_nItemId@19 均非对齐
struct PacketHeader
{
    unsigned short m_wMessageID;
    unsigned short m_wSize;
    unsigned short m_wSrcPort;
    // ORIG DWARF：@6 是 unsigned int m_dwSrcIp（原误写为两个 ushort）
    unsigned int m_dwSrcIp;
...
{
    // ORIG：单条 OR 链 + 共享 return true（jle 逐区间跳过，末段跳共享块）
    if ((0x3a97 < category && category <= 0x3dc3)
        || (0x4267 < category && category <= 0x4593)
        || (0x4a37 < category && category <= 0x4d63)
        || (0x5207 < category && category <= 0x5533)
        || (0x59d7 < category && category <= 0x5d03)
        || (0x61a7 < category && category <= 0x64d3))
    {
        return true;
    }
    return false;
...
{
    // ORIG DWARF 声明序：new_emblem_info(770) 先于 iter(772)。
...
{
    // ORIG DWARF 声明序：new_expansion_info 先于 iter。
...
__attribute__((target("arch=i586"))) int Auction::RegistItem(
    int ownerId, const char* ownerName, char ownerType, int price, int instantPrice,
    unsigned int black_point, int unitPrice, const ROI_Category& roiKey,
    char* ownerNexonId, DnfItemInfo itemInfo, unsigned long long& auctionId,
    long expireTime, int buyerId, const char* buyerName, bool isLoad)
{
    int return_code = 0;
    int now_registed_item_num = 0;
    if (GetItemInfo(itemInfo.item_id) == (CNRDItemInfoList::STItemInfo*)0x0)
    {
        return 0;
    }
...
int Auction::AddItemAveragePrice(unsigned long itemId, unsigned char itemUpgradeValue,
                                 int price, unsigned int real_pur_cnt,
                                 const ROI_AverageKey& _roi_average_key,
                                 unsigned char itemRefineValue, bool loadFromDb)
{
    int return_code = 0;
    bool first_add = false;
    int average_price = 0;
    CNRDItemInfoList::STItemInfo* pItemInfo = GetItemInfo(itemId);
    if (pItemInfo != NULL)
    {
        bool is_stack = IsStackableCategory(pItemInfo->category_);
        return_code = mAuctionDic.AddItemAveragePrice(
            itemId, itemUpgradeValue, price, first_add, average_price, is_stack,
            _roi_average_key, real_pur_cnt, itemRefineValue);
    }
    if (return_code != 0)
    {
    ...
    if (!loadFromDb)
    {
        if (first_add)
        {
            ...
            dbtr_insert_average_price.item_id = itemId;
            dbtr_insert_average_price.upgrade = itemUpgradeValue;
            dbtr_insert_average_price.average_price = average_price;
            dbtr_insert_average_price._roi_average_key = _roi_average_key;
            dbtr_insert_average_price.seperate_upgrade = itemRefineValue;
            Message* pMsg =
                pApp->super_DataPools.getCommonDataPool(tlsThreadId)->createMessage(3);
            CMsgCell* pNewCell = pMsg->getCellFromMessage();
        ...
        else
        {
            ...
            dbtr_update_average_price.item_id = itemId;
            dbtr_update_average_price.upgrade = itemUpgradeValue;
            dbtr_update_average_price.average_price = average_price;
            dbtr_update_average_price._roi_average_key = _roi_average_key;
            dbtr_update_average_price.purchase_cnt = real_pur_cnt;
            dbtr_update_average_price.seperate_upgrade = itemRefineValue;
            Message* pMsg =
                pApp->super_DataPools.getCommonDataPool(tlsThreadId)->createMessage(3);
            CMsgCell* pNewCell = pMsg->getCellFromMessage();
...
int Auction::Set_ROI_Constraint(const ROI_Average_Constraint& constraint)
{
    int return_code = 0;
    return_code = mAuctionDic.dic_Set_ROI_Constraint(constraint);
    return return_code;
}
...
int Auction::TransErrToReason(int err_no)
{
    switch (err_no)
    {
    // ORIG 的 case 处理器顺序（源码书写序，非升序）：
    // 0x24,0x2b,0x2d,0x2e,0x2f,0x23,0x31,0x1f/0x26,0x37。
    case 0x24:
        return 0x96;
    case 0x2b:
        return 0x9a;
...
{
    int error_no = 0;
    error_no = mAuctionDic.GetAveragePrice(item_id, upgrade, roiKey, refine, out);
    if (error_no != 0)
    {
                            ...
                            AuctionItemInfo* pOutAuctionItemInfoArray)
{
    ...
    {
        // ORIG：error_no==0 路径显式 mov $0,%eax（+1 条），否则 je 直跳 epilogue。
        return 0;
    }
    return error_no;
}
                              ...
                              AuctionItemInfo* pOutAuctionItemInfoArray)
{
    ...
    {
        // ORIG：同 SearchByItemId，error_no==0 显式返回 0。
        return 0;
    }
    return error_no;
}
...
{
    int error_no = 0;
    error_no = mAuctionDic.RegistCancel(owner_id, auction_id);
    if (error_no != 0)
    {
...
void Auction::OpenPrivateStore(unsigned int m_id, int character_num, int* character_ids)
{
...
{
    int error_no = 0;
    error_no = mAuctionDic.GetRegistedItemInfo(owner_id, pNum, pOut);
    if (error_no != 0)
    {
...
{
    int error_no = 0;
    error_no = mAuctionDic.GetBiddingInfo(buyer_id, pNum, pOut);
    if (error_no != 0)
    {
...
void Auction::ClosePrivateStore(unsigned int m_id)
{
    std::map<unsigned int, std::vector<int> >::iterator iter = mIdMap.find(m_id);
    if (iter == mIdMap.end())
    {
                     ...
                     int price, char* out, int& ret)
{
    ...
    if (price < 1)
    {
        // ORIG：GetErrorStr(return_code)（此时为 0）；return 0x25 直出 eax
        G_TraceLog()->sysLog(5, "Auction::Bidding(), price, %s",
                             GetErrorStr(return_code));
        return 0x25;
    }
    else
...
bool Auction::IsOwnerVIP(unsigned long long auction_id, OwnerInfo& ownerInfo)
{
...
EnumAuctionItemType Auction::CheckItemType(unsigned long item_id)
{
    if (item_id != 0)
    {
        int category = (int)GetItemInfo(item_id)->category_;
...
{
    // ORIG：单条 OR 链 + 共享 return true
    if ((0x4a37 < category && category <= 0x4d63)
        || (0x5207 < category && category <= 0x5533)
        || (0x59d7 < category && category <= 0x5d03)
        || (0x61a7 < category && category <= 0x64d3))
    {
...
{
    // ORIG：if (emblemSocketType_ == 0) return true;（then 在 fall-through，jne 直跳 switch）
