...
void ActiveNetClose::pushActiveClose(TCPUser* pUser)
{
    ...
    unsigned int id = pUser->getUserId();
    activeCloseMap[id] = pUser;
    bInData = true;
    G_TraceLog()->sysLog(7, "active close push user tcpuserid :(%d) map size:(%d)", id, (int)activeCloseMap.size());
}
...
bool ActiveNetClose::onActvieClose(unsigned int& outId, TCPUser*& outUserPtr)
{
    ...
    if (pUser->onActiveClose(key))
    {
        ...
        outUserPtr = pUser;
        ret_val = true;
        G_TraceLog()->sysLog(7, "onActiveClose erase tcpuserid :(%d) map size:(%d)", key, (int)activeCloseMap.size());
    }
