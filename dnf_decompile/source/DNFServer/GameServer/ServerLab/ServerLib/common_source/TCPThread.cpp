...
void TCPThread::loop(void* temp)
{
    ...
    if ((G_Script()->findIntValue(0, 4) == 0x5209) || r->registListenHandle(sUser))
    {
        while (true)
        {
            do
            {
                ...
                {
                    unsigned int active_close_user_id = 0;
            ...
            } while ((unsigned long long)(now_time - last_heartbeat_check_time) < (unsigned long long)HEART_BEAT_CHECK_INTERVAL);
            {
                ...
                {
                    if ((tcp_send_thread != NULL) && (!client_iter->second->isAboutToDisconnect()))
                    {
