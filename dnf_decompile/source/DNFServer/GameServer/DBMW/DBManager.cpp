                                     ...
                                     char* newMasterName)
{
    ...
    CDBHandle* unused = m_handles[2];    // unused local matching ORIG stack
    (void)unused;
    ...
    {
        char* midStr = NumberToString(newMasterMId, 0);
        ...
        log("./log/DBQueryErr",
            ...
            midStr, newMasterNo, newMasterName,
            guildId, serverId);
        return 0;
    }
    ok = h->exec(0x4e6a);
    ...
    Packet_DB_Load_Request_Guild_Board_Write* req, STGuildBoardDBInfo* info)
{
    ...
    if (!h->fetch())
        return 0;
    if (!h->get_uint(0, *(unsigned int*)&info->m_no))
        return 0;
    ...
    STGuildBoardDBInfo* info)
{
    ...
    if (!h->get_uint(0, *(unsigned int*)&info->m_no))
        return 0;
    if (!h->get_uint(1, *(unsigned int*)&info->m_characNo))
        return 0;
    if (!h->get_str(2, info->m_member.m_name, 0x1e))
        return 0;
    if (!h->get_str(3, (char*)info, 0x78))
        return 0;
    if (!h->get_uint(4, *(unsigned int*)&info->m_createTime))
        return 0;
    if (!h->get_byte(5, *(char*)&info->m_member.m_job))
        return 0;
    return 1;
}
                                 ...
                                 Packet_Guild_Load_Guild_Agit& reply)
{
    ...
    r = h->get_ubyte(0, *(unsigned char*)((char*)&reply + 0xe));
    if (!r)
    {
    ...
    Packet_DB_Insert_Guild_Cargo_History* req)
{
    ...
    CDBHandle* h = m_handles[8];    // guild db
    char table[0xff] = {0};
    sprintf(table, "guild_cargo_history_%d", req->m_guildId % 10);
    h->set_query(
        0x4ed9,
        ...
        req->m_characName, req->m_slotNo,
        req->m_movetoSlotNo, req->m_behavior,
        (unsigned char)req->m_item.m_field0, req->m_item.m_field1,
        req->m_item.m_field6, req->m_item.m_fieldA,
        req->m_item.m_fieldC,
        ...
        (unsigned char)req->m_item.m_field5 & 0x1f,
        (unsigned char)req->m_item.m_field5 / 0x20,
        (unsigned char)req->m_item.m_field10, req->m_item.m_field11,
        h->blob_to_str(0, (char*)&req->m_item.m_option, 0xe),
        req->m_item.m_upg.GetUpgradeSeparate());
    r = h->exec(0x4ed9);
    if (!r)
    {
...
bool CDBManager::QueryBlackList(unsigned int m_id, STBlackUserDBType* list)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (!ret)
            return 0;
        ret = h->get_uint(0, *(unsigned int*)&list[i].m_characNo);
        if (!ret)
            return 0;
        ret = h->get_str(1, list[i].m_name, 0x1e);
        if (!ret)
            return 0;
        ret = h->get_uint(2, *(unsigned int*)&list[i].m_occTime);
        if (!ret)
            return 0;
    }
    return 1;
                                  ...
                                  Packet_DB_Reply_Query_Guild_Member& reply)
{
    ...
    r = h->get_uint(0, *(unsigned int*)&reply.m_guildId);
    if (!r)
    {
        reply.m_flag = 3;
        return 0;
    }
    r = h->get_str(1, reply.m_info.m_memo, 0x15);
    if (!r)
    {
        reply.m_flag = 3;
        return 0;
    }
    r = h->get_ubyte(2, *(unsigned char*)&reply.m_info.m_grade);
    if (!r)
    {
        reply.m_flag = 3;
        return 0;
    }
    r = h->get_uint(3, *(unsigned int*)&reply.m_info.m_memberPoint);
    if (!r)
    {
        reply.m_flag = 3;
        return 0;
    ...
    Packet_StartGameEventFromServer* packet)
{
    ...
    if (!ok)
    {
        const char* q = h->get_quest_str();
        CMyFileLog log(__FUNCTION__, 0x2e96);
        log("./log/DBQueryErr", q);
        return 0;
    }
    ok = h->exec(0x4f5d);
    if (!ok)
...
char CDBManager::UpdateGuildRank(int serverId, CGuildManager* gm)
{
         ...
         it != rankList->end(); ++it)
    {
        ...
        if (!h->set_query(0x4e34,
                          ...
                          info->m_rank, info->m_guildId, serverId))
        {
    ...
    Packet_DB_Query_Reply_On_Guild_Booting& reply, int serverId)
{
    ...
    if (h->get_n_rows() != 0)
    {
        ...
        if (!r)
            return 0;
        r = h->get_int(0, *(int*)&reply.m_aSidePoint);
        if (!r)
            return 0;
        r = h->get_int(1, *(int*)&reply.m_bSidePoint);
        if (!r)
            return 0;
        r = h->get_byte(2, *(char*)&reply.m_winnerSide);
        if (!r)
            return 0;
    }
    return 1;
    ...
    Packet_DB_Write_Guild_Member_Memo* packet)
{
    bool r;
    CDBHandle* h = m_handles[8];    // guild db
    unsigned int guildId = packet->m_guildId;
    unsigned int characNo = packet->m_characNo;
    char buf[0x6002];
    memset(buf, 0, 0x6002);
    h->escape_string(buf, packet->m_memo);
...
{
    m_hackType = other.m_hackType;
    m_cnt = other.m_cnt;
    m_etc = other.m_etc;
    m_hackSubType = other.m_hackSubType;
    m_hackSubCnt = other.m_hackSubCnt;
    m_applyFlag = other.m_applyFlag;
    m_ipCnt = other.m_ipCnt;
}
...
Packet_CollectItemsResult::Packet_CollectItemsResult()
    : PacketHeader(0x27e7, 0x16), m_totalCount(0), m_curCount(0), m_changeFlag(0) {}
Packet_DB_Reply_Query_Guild::Packet_DB_Reply_Query_Guild()
    : PacketHeader(0x405, 0x135), m_result(0), m_guildId(0), m_characNo(0), m_guildInfo()
{
...
Packet_Guild_Load_Guild_Agit::Packet_Guild_Load_Guild_Agit()
    : PacketHeader(0x6e1, 0xf), m_guildId(0), m_info() {}
...
Packet_DB_Response_Approve_Join_Guild::Packet_DB_Response_Approve_Join_Guild()
    : PacketHeader(0x1bc5, 0x56), m_result(0), m_guildId(0), m_id(0), m_characNo(0)
{
    memset(&m_joinInfo, 0, sizeof(m_joinInfo));
}
Packet_Guild_Load_Guild_Cargo_History::Packet_Guild_Load_Guild_Cargo_History()
    : PacketHeader(0x709, 0x972), m_guildId(0), m_count(0)
{
...
Packet_DB_Monitor_Change_Unconnected_GuildMember_Grade::
    Packet_DB_Monitor_Change_Unconnected_GuildMember_Grade()
    : PacketHeader(0x42b, 0x36), m_guildId(0), m_characNo(0), m_result(0xff), m_grade(0), m_newGrade(0)
{
    memset((char*)this + 0x12, 0, 0x1e);
}
Packet_DB_Reply_Query_Member::Packet_DB_Reply_Query_Member()
    : PacketHeader(0x4b3, 0x1c5), m_flag(0), m_characNo(0), m_master() {}
Packet_DB_Reply_Unconn_Guild_Member::Packet_DB_Reply_Unconn_Guild_Member()
    : PacketHeader(0x427, 0x53), m_guildId(0), m_characNo(0), m_proxy() {}
Packet_DB_Reply_Guild_All_Members::Packet_DB_Reply_Guild_All_Members()
    : PacketHeader(0x426, 0x17b1), m_guildId(0), m_characNo(0), m_flag(0), m_count(0) {}
                                       ...
                                       STGuildMemberProxy& proxy)
{
    ...
    if (!h->get_str(1, proxy.m_name, 0x1d))
        return 0;
    if (!h->get_byte(2, (char&)proxy.m_job))
        return 0;
    if (!h->get_short(3, (short&)proxy.m_lev))
        return 0;
    if (!h->get_byte(4, (char&)proxy.m_growType))
        return 0;
    if (!h->get_byte(5, (char&)proxy.m_sex))
        return 0;
    return 1;
}
                                           ...
                                           unsigned short& count)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (!h->get_str(1, proxies[i].m_name, 0x1d))
            return 0;
        if (!h->get_byte(2, (char&)proxies[i].m_job))
            return 0;
        if (!h->get_short(3, (short&)proxies[i].m_lev))
            return 0;
        if (!h->get_byte(4, (char&)proxies[i].m_growType))
            return 0;
        if (!h->get_byte(5, (char&)proxies[i].m_sex))
            return 0;
        if (!h->get_ubyte(6, proxies[i].m_grade))
            return 0;
        if (!h->get_uint(7, (unsigned int&)proxies[i].m_lastPlayTime))
            return 0;
        if (!h->get_str(8, proxies[i].m_memo, 0x15))
            return 0;
    }
    return 1;
}
...
#define PM(i) ((STPartyMemberStat*)((char*)packet + 0x10 + (i) * 0x3c))
        if (sql.size())
            sprintf(buf,
                    ...
                    PM(i)->m_channelNo, PM(i)->m_dungeonIndex, PM(i)->m_dungeonDiff,
                    PM(i)->m_dungeonStandardLevel, PM(i)->m_abuseParty, PM(i)->m_balkunParty,
                    PM(i)->m_success, PM(i)->m_partyUserCount, PM(i)->m_clearTime,
                    PM(i)->m_dieCount, PM(i)->m_hpConsume, PM(i)->m_mpConsume,
                    PM(i)->m_hitCount, PM(i)->m_hitPerAvgDamage, PM(i)->m_hpRecovery,
                    PM(i)->m_mpRecovery, PM(i)->m_updateCount, PM(i)->m_level,
                    PM(i)->m_fatigueConsume, PM(i)->m_expAdd);
        else
            sprintf(buf,
                    "(now(),%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d)",
                    PM(i)->m_channelNo, PM(i)->m_dungeonIndex, PM(i)->m_dungeonDiff,
...
#define PM(i) ((STPartyJobMemberStat*)((char*)packet + 0x10 + (i) * 0x19))
        if (sql.size())
            sprintf(buf,
                    ...
                    PM(i)->m_channelNo, PM(i)->m_dungeonIndex, PM(i)->m_dungeonDiff,
                    PM(i)->m_dungeonStandardLevel, PM(i)->m_abuseParty, PM(i)->m_balkunParty,
                    PM(i)->m_success, PM(i)->m_partyUserCount, PM(i)->m_characJob,
                    PM(i)->m_characGrow, PM(i)->m_jobCount, PM(i)->m_rank);
        else
            sprintf(buf,
                    "(now(),%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d)",
                    PM(i)->m_channelNo, PM(i)->m_dungeonIndex, PM(i)->m_dungeonDiff,
...
#define PM(i) ((STPartyCharacMemberStat*)((char*)packet + 0x10 + (i) * 0x43))
        if (sql.size())
            sprintf(buf,
                    ...
                    PM(i)->m_channelNo, PM(i)->m_dungeonIndex, PM(i)->m_dungeonDiff,
                    PM(i)->m_dungeonStandardLevel, PM(i)->m_success, PM(i)->m_characJob,
                    PM(i)->m_characGrow, PM(i)->m_clearTime, PM(i)->m_dieCount,
                    PM(i)->m_hpConsume, PM(i)->m_mpConsume, PM(i)->m_hitCount,
                    PM(i)->m_hitPerAvgDamage, PM(i)->m_hpRecovery, PM(i)->m_mpRecovery,
                    PM(i)->m_updateCount, PM(i)->m_level, PM(i)->m_fatigueConsume,
                    PM(i)->m_expAvg, PM(i)->m_partyUserCount, PM(i)->m_rank);
        else
            sprintf(buf,
                    "(now(),%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d)",
                    PM(i)->m_channelNo, PM(i)->m_dungeonIndex, PM(i)->m_dungeonDiff,
...
bool CDBManager::QueryGuildWarPointList(int guildWarPoint, CGuildManager* gm)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (!info)
            return 0;
        cVar1 = h->get_uint(0, info->m_guildId);
        if (!cVar1)
            return 0;
        cVar1 = h->get_uint(1, info->m_guildWarPoint);
        if (!cVar1)
            return 0;
        cVar1 = h->get_str(2, info->m_name, 0x17);
        if (!cVar1)
            return 0;
        cVar1 = h->get_uint(3, info->m_guildPointPrev);
        if (!cVar1)
            return 0;
        ranks->push_back(std::make_pair(info->m_guildWarPoint, info));
    }
    return 1;
}
                                     ...
                                     char* characName)
{
    ...
    {
        char* midStr = NumberToString(m_id, 0);
    ...
    Packet_Item_Limit_Edition_Load_Data_Rpy* rpy)
{
    ...
    time_t now = time(0);
    char buf[0x1000] = {0};
    if (req->m_ipgNoCount != 0 || req->m_flag != 1)
    {
...
#define QG_FAIL() \
    ...
    r = h->get_str(0, info->m_guildName, 0x17);
    if (!r)
        QG_FAIL();
    r = h->get_uint(1, *(unsigned int*)&info->m_masterNo);
    if (!r)
        QG_FAIL();
    r = h->get_ubyte(2, *(unsigned char*)&info->m_lev);
    if (!r)
        QG_FAIL();
    r = h->get_ushort(3, *(unsigned short*)&info->m_ability);
    if (!r)
        QG_FAIL();
    r = h->get_ushort(4, *(unsigned short*)&info->m_memberCount);
    if (!r)
        QG_FAIL();
    r = h->get_uint(5, guildRank);
    if (!r)
        QG_FAIL();
    if (guildRank > 0x64)
        info->m_guildRank = 0;
    ...
    r = h->get_uint(6, *(unsigned int*)&info->m_guildPoint);
    if (!r)
        QG_FAIL();
    r = h->get_uint(7, *(unsigned int*)&info->m_guildExp);
    if (!r)
        QG_FAIL();
    r = h->get_ubyte(8, *(unsigned char*)&info->m_powerSide);
    if (!r)
        QG_FAIL();
    r = h->get_uint(9, *(unsigned int*)&info->m_powerSecedeTime);
    if (!r)
        QG_FAIL();
    r = h->get_uint(10, *(unsigned int*)&info->m_powerWarPoint);
    if (!r)
        QG_FAIL();
    r = h->get_ubyte(11, *(unsigned char*)&info->m_guildAgitFlag);
    if (!r)
        QG_FAIL();
    r = h->get_ubyte(12, *(unsigned char*)&info->m_powerJoinCount);
    if (!r)
        QG_FAIL();
    r = h->get_uint(13, *(unsigned int*)&info->m_guildFund);
    if (!r)
        QG_FAIL();
    r = h->get_str(14, info->m_masterName, 0x15);
    if (!r)
        QG_FAIL();
#undef QG_FAIL
    reply.m_result = 1;
    ...
    Packet_DBMW_Statistic_Login_Logout* packet)
{
    ...
    h->set_query(0x4eec,
                 ...
                 packet->m_numOccupationsCharscreen, packet->m_numOccupationsSeriaroom,
                 packet->m_numLoginPerMin, packet->m_numLogoutPerMin);
    ret = h->exec(0x4eeb);
    if (!ret)
    {
...
char CDBManager::QueryOnTimeEventIdx(Packet_Result_OnTimeEvent_Idx& rpy)
{
    ...
    if (!h->exec(0x4f14) || !h->fetch())
        return 0;
    if (!h->get_uint(0, *(unsigned int*)&rpy.m_maxNo))
        return 0;
    return 1;
}
                                   ...
                                   CStatisticsServer* stats)
{
    ...
    if (h->get_n_rows() != 0)
    {
        if (!h->fetch())
            return 0;
        if (!h->get_short(0, *(short*)&pkt.m_intervalSec))
            return 0;
    }
    else
    {
                                       ...
                                       Packet_Reply_Today_Guild_Member& reply)
{
    ...
    while (i < (unsigned int)h->get_n_rows())
    {
        ...
        if (!h->fetch())
            return 0;
        if (!h->get_uint(0, p->m_characNo))
            return 0;
        if (!h->get_str(1, p->m_name, 0x1d))
            return 0;
        if (!h->get_byte(2, *(char*)&p->m_grade))
            return 0;
        if (!h->get_byte(3, *(char*)&p->m_job))
            return 0;
        if (!h->get_byte(4, *(char*)&p->m_growType))
            return 0;
        if (!h->get_byte(5, *(char*)&p->m_sex))
            return 0;
        if (!h->get_byte(6, *(char*)&p->m_lev))
            return 0;
        vec.push_back(*p);
        i++;
    }
    ...
    unsigned int guildId, Packet_Guild_Load_Guild_Cargo_History& reply)
{
    ...
    while (j < (unsigned int)h->get_n_rows())
    {
        if (!h->fetch())
            return 0;
        col = 0;
        if (!h->get_int(col++, *(int*)&reply.m_logs[j].time))
            return 0;
        if (!h->get_byte(col++, *(char*)&reply.m_logs[j].behavior))
            return 0;
        if (!h->get_str(col++, reply.m_logs[j].name, 0x15))
            return 0;
        if (!h->get_int(col++, *(int*)&reply.m_logs[j].count))
            return 0;
        if (!h->get_int(col++, *(int*)&reply.m_logs[j].param))
            return 0;
        if (!h->get_binary(col++, (char*)&reply.m_logs[j].opt0, 0xe))
            return 0;
        j++;
    }
    return 1;
                                  ...
                                  STGuildBoardDBInfo* boards)
{
    ...
    while (i < (unsigned int)count)
    {
        ...
        if (!h->get_uint(0, *(unsigned int*)&b->m_no))
            return 0;
        if (!h->get_uint(1, m_id))
            return 0;
        if (!h->get_uint(2, *(unsigned int*)&b->m_characNo))
            return 0;
        if (!h->get_str(3, b->m_member.m_name, 0x1e))
            return 0;
        if (!h->get_str(4, b->m_pre, 0x78))
            return 0;
        if (!h->get_uint(5, *(unsigned int*)&b->m_createTime))
            return 0;
        if (!h->get_byte(6, *(char*)&b->m_member.m_job))
            return 0;
        if (m_id == 0)
            b->m_characNo = 0;
        i++;
    ...
    Packet_UnChangable_GuildInfo_Save* packet)
{
    ...
    {
        int characNo = packet->m_characNo;
        int guildId = packet->m_guildId;
    ...
    std::vector<st_full_ip_counter_list>& fullIpList)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (!h->get_ushort(0, item.m_hackType))
            return 0;
        if (!h->get_ushort(1, item.m_hackSubType))
            return 0;
        memset(item.m_cClassIp, 0, sizeof(item.m_cClassIp));
        if (!h->get_str(2, item.m_cClassIp, 0xc))
            return 0;
        if (!h->get_uint(3, item.m_cnt))
            return 0;
...
bool CDBManager::QueryLoadARSInfo(std::vector<st_ars_info_list>& arsList)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (!h->get_ushort(0, item.m_hackType))
            return 0;
        if (!h->get_ushort(1, item.m_cnt))
            return 0;
        // 注意：以下列序与 SQL 列名不完全对齐，但这是 ORIG 的原始行为
        //（本函数与 ORIG 逐字节 IDENTICAL，勿按 SQL 列名"修正"读取顺序）。
        // col2(etc)→+0xa、col3(hack_sub_type)→+4、col4(hack_sub_cnt)→+6、
        // col5(apply_flag)→+8、col6(ip_cnt)→+9。
        if (!h->get_ushort(2, item.m_ipCnt))
            return 0;
        if (!h->get_ushort(3, item.m_etc))
            return 0;
        if (!h->get_ushort(4, item.m_hackSubType))
            return 0;
        if (!h->get_ubyte(5, item.m_hackSubCnt))
            return 0;
        if (!h->get_ubyte(6, item.m_applyFlag))
            return 0;
                                     ...
                                     Packet_DB_Reply_Query_Guild& reply)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (!h->get_uint(0, *(unsigned int*)&info->m_subMasterData[i * 4]))
        {
            reply.m_result = 3;
            return 0;
        }
...
char CDBManager::QueryOnTimeEventItem(Packet_Result_Ontime_Event_Item& reply)
{
    ...
    if (!h->get_uint(0, *(unsigned int*)&reply.m_idx))
        return 0;
    if (!h->get_uint(1, *(unsigned int*)&reply.m_cnt))
        return 0;
    return 1;
}
                                    ...
                                    int& out2)
{
    ...
    int param1 = 0;
    int param2 = 0;
    ok = h->get_uint(0, logId);
    if (!ok)
        return 0;
    ok = h->get_int(1, param1);
    if (!ok)
        return 0;
    ok = h->get_int(2, param2);
    if (!ok)
        return 0;
    param1 += add;
    if (param1 < 0)
...
bool CDBManager::updateLimitNpcBuyItemInfo(LimitNpcBuyItemUpdate* update)
{
    ...
    ok = h->set_query(0x4f46,
                      ...
                      update->m_sellCountInc, update->m_itemIndex);
    if (!ok)
    {
                                             ...
                                             unsigned int& result)
{
    ...
    if (!r)
        return 0;
    r = h->get_ubyte(1, grade);
    if (!r)
        return 0;
    r = h->get_uint(2, result);
    if (!r)
        return 0;
    return 1;
}
                                       ...
                                       Packet_DB_Reply_Query_Guild& reply)
{
    ...
    if (!r)
        return 1;
    r = h->get_str(0, (char*)&reply + 0xd0, 0x64);
    if (!r)
        return 0;
    return 1;
}
                                 ...
                                 Packet_DB_Reply_Query_Guild& reply)
{
    ...
    r = h->get_ushort(0, *(unsigned short*)&info->m_guildSkillPoint);
    if (!r)
    {
        reply.m_result = 3;
        return 1;
    }
    r = h->get_ubyte(1, *(unsigned char*)&info->m_skillLearnCnt);
    if (!r)
    {
        reply.m_result = 3;
        return 1;
    }
    if ((unsigned char)info->m_skillLearnCnt != 0)
    {
        r = h->get_binary(2, (char*)info->m_skills,
                          (unsigned char)info->m_skillLearnCnt * 5);
        if (!r)
        {
            reply.m_result = 3;
