# guild 第 25 轮类粒度回归 —— 子 Agent 通用手册（2026-08-12）
...
- 权威基线：guild 非 identical = **328**（DIFF 317 + NEAR 11；IDENTICAL 934 /
  IDENTICAL_AE 288）。你组清单见 `round25_groups/<group>.list`（mangled 名，一行为一个）。
...
- **行号 / 偏移 / 常量差异是真实差异**：`__assert_fail` / 日志宏行号实参两侧
  不同必须按 ORIG 实测值对齐源码行号（可用 `source/toolchain/align_log_lines.py`
  思路）；字段偏移、魔数必须以 ORIG 二进制实测为准，不得凭 DWARF/Ghidra 猜测。
...
## 7. 分组与 TU 归属（round25_groups/*.list）
