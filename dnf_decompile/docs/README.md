...
| `df_bridge_r_restoration_report.md` | **df_bridge_r（bridge 服务）还原报告**：31 TU 全部编译链接通过；与 channel 交叉比对（TU 级 + 类布局）；746 个项目函数符号 100% 对齐，助记符级精确 **93.4%**（692 IDENTICAL + 5 NEAR / 49 DIFF / 0 MISSING），不低于 community 水位；49 个 DIFF 的语义等价分类说明 | 报告 |
