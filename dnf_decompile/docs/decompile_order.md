...
生成时间：2026-08-06（2026-08-08 更新：新增 `game`、`secsvr`（gunnersvr/zergsvr/secagent）、`dbmw` 三组二进制的基本信息与队列位置，详见 `df_game_secsvr_dbmw_basic_info.md`；2026-08-09 更新：auction/point 已由 2026-08-07~08-08 还原会话（session `2c0a7ae8-2b97-4d37-b768-a07b8c1e9266`）完成并移出队列，guild/monitor 现有进度刷新，`df_game_r` 明确固定为队尾最低优先级；secsvr 三件套已完成 DWARF 桩还原（文件镜像 + 类型信息 + 参数/变量名，函数体未动）后**移出本队列**，独立工作区 `../dnf_secsvr/` 专门还原，全部上下文文档随迁）
...
- **`secsvr` 三件套（gunnersvr/zergsvr/secagent）已移出本队列**：源码独立（`/data/secci/ci/.../g3_release_suse32*`），共享 `commlib/framework` + `commlib/zenlib` + `protocol/common`；还原在独立工作区 `../dnf_secsvr/` 进行，不再在本文档维护。
...
- `df_game_r` 已加入 Ghidra 工程 `dnf_project`（2026-08-08 导入；`gunnersvr`/`zergsvr`/`secagent` 同批导入，其分析见独立工作区 `../dnf_secsvr/`）。
...
| 1 | `relay` | 418 KB | 1,921 | 3.1k | 无 | 主体实现（应用级 MISSING=0） | 无 DWARF 中最小的入门目标；real missing 15 均为 std/工具链细节 |
| 2 | `coserver` | 1.1 MB | 2,987 | 6.1k | 无 | 应用层主体完成 | 功能单一（阻止重复连线） |
| 3 | `statics` | 1.7 MB | 6,059 | 9.7k | 无 | 主体实现（符号命中 99.6%） | 统计服务 |
| 4 | `guild` | 2.3 MB | 7,024 | 12.3k | 无 | 应用层缺失归零（MISSING 2，仅编译器局部 clone） | 公会操作 |
| ~~4~~ | ~~`monitor`~~ | 2.5 MB | 7,899 | 13.3k | 无 | ✅ 已完成（MISSING=0，见 `df_monitor_r_progress.md` / `df_monitor_r_validation.md`） | 无 DWARF 中函数最多 |
| 5 | `manager` | 3.2 MB | 5,948 | 25.8k | 无 | ✅ 主体完成（MISSING=0；助记符重叠 88.96%、字符串交集 90.3%，见 `df_manager_r_validation.md`） | 字符串量最大，消息/管理面广 |
| 6 | `dbmw` | 4.0 MB | 7,816 | 32.1k | 几乎无效（2 CU） | 占位 | 无 DWARF 中体量最大，DB 中间件；三副本 md5 相同只需还原一次 |
| 7 | `df_game_r`（game） | 36.4 MB | 116,059（含静态库） | 212.0k | 部分（21 CU，核心无） | 已导入 | 体量最大；`../../Include/` 走 DWARF 补桩、GameServer 核心走 Ghidra、FreeType 黑盒；**优先级最低（固定队尾，不因其他服务完成而提前）** |
...
5. **`df_game_r`（game）固定为全队列最低优先级**：体量最大（36.4 MB / 11.6 万函数）、核心无 DWARF、Ghidra 分析面巨大（约 9.8 万函数），始终排在队尾（排序表第 7 位、最后一行），不因其他服务完成而提前。
...
- 注意工具链差异：`game`/`dbmw` 为 4.1.2 + 4.4.x 混合；构建基线需按目标单独评估（stun 已确立 64 位特例先例；secsvr 的 GCC 4.1.0 SUSE 基线在独立工作区 `../dnf_secsvr/` 评估）。
- 每补完一个服务即可纳入 CMake 构建（先补 `main` 使目标可链接）。
- 构建基线保持 C++98 / 32 位 / O0；`stun` 需单独评估 64 位基线。

### 无有效 DWARF 组：Ghidra 路线

- 走 Ghidra Headless 导出伪 C → 逐函数人工比对的流程（`/mnt/d/Works/ghidra/support/analyzeHeadless`，现有工程 `dnf_project` 已导入 `df_game_r` 等程序；secsvr 三件套的 Ghidra 分析见独立工作区 `../dnf_secsvr/`）。
...
- secsvr 三件套（已移出队列）：独立工作区 `../dnf_secsvr/`（桩源码 + 还原报告 + 专属顺序文档）
