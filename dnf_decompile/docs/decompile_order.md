...
生成时间：2026-08-06（2026-08-07 更新：`stun`、`channel` 已完成还原，移出队列；`bridge` 升为下一目标）

本文档基于对 `dnf_installer/build/dnf_data/home/template/neople/` 下全部 13 个唯一服务端二进制（15 个文件，dbmw 三副本）的实测指标，给出从易到难的还原顺序与策略。`df_community_r`、`stun`、`channel` 已完成主体重建并作为对照基准，不再列入队列。
...
| 2 | `bridge` | 3.4 MB | 3,812 | 30.2k | 完整（31 CU） | DWARF 桩 | **下一目标**：与 channel 共享框架类，可整体复用 channel 已对齐实现，再补专属逻辑 |
| 3 | `auction`/`point`（一组） | 12.2 MB | 8,205 ×2 | 84.7k | 完整（66 CU） | 桩已生成 | 体量大但元数据最全；还原一个得两个 |
| 4 | `relay` | 418 KB | 1,921 | 3.1k | 无 | 占位 | 无 DWARF 中最小的入门目标 |
| 5 | `coserver` | 1.1 MB | 2,987 | 6.1k | 无 | 占位 | 功能单一（阻止重复连线） |
| 6 | `statics` | 1.7 MB | 6,059 | 9.7k | 无 | 占位 | 统计服务 |
| 7 | `guild` | 2.3 MB | 7,024 | 12.3k | 无 | 占位 | 公会操作 |
| 8 | `monitor` | 2.5 MB | 7,899 | 13.3k | 无 | 占位 | 无 DWARF 中函数最多 |
| 9 | `manager` | 3.2 MB | 5,948 | 25.8k | 无 | 占位 | 字符串量最大，消息/管理面广 |
| 10 | `dbmw` | 4.0 MB | 7,816 | 32.1k | 几乎无效（2 CU） | 占位 | 无 DWARF 中体量最大，DB 中间件 |
