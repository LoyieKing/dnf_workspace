# guild 第 30 轮类粒度回归 —— 子 Agent 通用手册（2026-08-12）
...
- 权威重建：`build/guild/df_guild_r`（guild 用 c6 变体：c6root g++ 4.4.7 驱动，
  ...
  `DNFUser.cpp` 与 `CFileLogWriterBase.cpp` 带 `-fno-exceptions`）。
- **第 30 轮权威基线（父 agent 已从 HEAD 全新重建并全量重生成）**：
  guild 非 identical = **273**（DIFF 254 + NEAR 19；IDENTICAL 959 /
  IDENTICAL_AE 318）。上一轮提交里的 md 集是对陈旧二进制测的；
  HEAD 源码已含全部修复，本轮按 273 起步。
- 你组清单见 `round30_groups/guild/<group>.list`（mangled 名，一行为一个）。
...
3. 重编 + 链接 scratch 二进制（只写你自己的 scratch 目录）：
   ...
   - 私有副本组（pt_a/pt_b/pt_c/guild_a/guild_b，同 TU 多组并行）：
     ...
     scratch_link <group>
     ```
     `/tmp/guild_base/*.cpp` 已由父 agent 在开工时刷新为当前 HEAD（含第 29 轮
     修复），私有副本务必从 `/tmp/guild_base` 复制。
...
## 7. 分组与 TU 归属（round30_groups/guild/*.list，由第 30 轮权威 manifest 生成）

| 组 | 函数数 | TU（basename） | 私有副本 |
|---|---|---|---|
| pt_a / pt_b / pt_c | 32/32/31 | DNFPacketTranslater | 是（每组一份） |
| guild_a / guild_b | 29/28 | DNFGuild | 是（每组一份） |
| guildmgr_srvhdr | 20 | DNFGuildManager + DNFServerHandler | 否（独占） |
| user_peer | 10 | DNFUser + Peer | 否（独占） |
| cargo_board | 14 | GuildCargo + GuildBoard | 否（独占） |
...
| xml_frame_sig | 23 | ServerXml + DNFTickHandler + DNFTableBase + DNFSignalTranslator + DNFApplication | 否（独占） |
| small_tu | 13 | DNFAppStartInit + DNFAppStopInit + DNFTcpSocket + MemoryCashManager + GMAccounts + DNFTcpNetworkThread + DNFNetworkThread + CashObject + TcpNetSystem | 否（独占） |
...
- 剩余 273 个绝大多数是 -O0 代码生成差异（栈槽/寄存器/求值顺序/寻址形态/
