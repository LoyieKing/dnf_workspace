# statics 第 30 轮类粒度回归 —— 子 Agent 通用手册（2026-08-12）
...
- 权威基线（本轮开始前已全量重建 + manifest + md 重生成）：非 identical = **66**
  （DIFF 64 + NEAR 2；IDENTICAL 558 / IDENTICAL_AE 219）。
  你组清单见 `round30_groups/statics/<group>.list`（mangled 名，一行为一个）。
...
| gm_accounts | `.../GMAccounts.cpp` | 2 | 独占 TU |
...
3. 重编 + 链接 scratch 二进制（只写你自己的 scratch 目录）：
   ...
   - **私有副本组**（stats_lifecycle/stats_add/stats_write/stats_senddb_a/
     ...
     **注意：`scratch_setup` 会 rm -rf 整个 scratch 目录，必须先 setup 再建
     ...
     scratch_link <group>
     ```
   - 逐 TU 工具链 A/B（当怀疑某函数 ORIG 编译器版本与 c6446r 不符，例如
     循环变量用 callee-saved 寄存器（`%esi/%ebx`）而非栈槽、双零扩展
     `movzbl %al,%edx` + 32 位 cmp、旧惯用法分支等形态）：
...
4. 复检：
   ...
   # 批量复检整组：
   python3 source/toolchain/statics_scratch_check.py /tmp/sub_statics_<group>/df_statics_r --file round30_groups/statics/<group>.list
   ```
...
- **行号 / 偏移 / 常量差异是真实差异**：`__assert_fail` / 日志宏行号实参两侧
  ...
  不得凭 DWARF/Ghidra 猜测。
- 基线源码说明：仓库 `Statistics.cpp` / `GMAccounts.cpp` 已含枚举限定名修复
  （`STPartyJobStatisticKey::STPartyJobKeyField10` 等）。**不要回退**；私有副本
  以 `/tmp/statics_base/Statistics.cpp` 为基线（已同步仓库最新版本）。

## 6. 可用纯 C++ 形态技巧（跨服务已验证）

- `if (x == -1) return;`、`switch(cond){case 0:break;case 1:break;}`（含 switch 单 case）
- `continue` / `goto LABEL` / 单 return 汇合 / 三元表达式 / 分支换向
- `register` 变量（阻止 4.4 消除临时拷贝；4.1.2 编译时 register 循环变量
  会落到 callee-saved 寄存器，正好匹配 ORIG 的 `%esi/%ebx` 循环形态）
