# monitor 第 26 轮类粒度回归 —— 子 Agent 通用手册（2026-08-12）
...
## 1. 背景与权威基线（2026-08-12 重新 manifest + 全量 md 测量）
...
- 权威基线（2026-08-12 14:24 重新 manifest + md 全量测量）：monitor 非 identical
  = **377**（NEAR 8 + DIFF 369；IDENTICAL 986 / IDENTICAL_AE 316 / EXEMPT_SKIP
  6247）。上一轮提交 380，本轮重建后基线 377，继续向 0 推进。
...
3. 重编 + 链接 scratch 二进制（只写你自己的 scratch 目录）：
   ...
   # 例：bash source/toolchain/monitor_scratch.sh /tmp/mon_r26_member_a_scratch DNFMemberManager
   ```

   - TU basename 不带 `.cpp`；一次可传多个 TU（改了几个 TU 就传几个，脚本只重编
     指定的，其余 .o 复用权威构建）。
   - 私有副本组（pt_a/pt_b/pt_c/pt_d，同 TU 多组并行）：编辑父 agent 给你的私有
     副本，再用 override 编译：
     ```bash
     export MON_SCRATCH_SRC_OVERRIDE=/tmp/mon_r26_pt_<X>/private/DNFPacketTranslater.cpp
     bash source/toolchain/monitor_scratch.sh /tmp/mon_r26_pt_<X>_scratch DNFPacketTranslater
     ```
...
4. 复检：
   ...
   #   --file round26_groups/monitor/<你的组>.txt
   # 查看完整伪代码 diff：
   python3 source/toolchain/gen_function_md.py --services monitor --function <mangled> \
       --new-bin <scratch_dir>/df_monitor_r
   ```
...
2. 你的源码改动 diff：
   - 独占 TU 组：`git diff -- source/DNFServer/GameServer/Monitor/<TU>.cpp` 直接贴出
     （你已直接改仓库文件；不要提交、不要 reset）。
   - 私有副本组：`diff -u /tmp/mon_r26_pt_<X>/private/DNFPacketTranslater.cpp` 相对
     基线（即父 agent 给的首版副本）的 diff；父 agent 会按函数合并。
3. 需要父 agent 决策的共享改动（头文件/布局/工具链 A/B 结论）。

## 7. 分组与 TU 归属（round26_groups/monitor/*.txt）

| 组 | 函数数 | TU（basename） | 私有副本 |
|---|---|---|---|
| pt_a / pt_b / pt_c / pt_d | 27/27/27/27 | DNFPacketTranslater | 是（/tmp/mon_r26_pt_a..d/private/） |
...
| network | 35 | TcpNetSystem + Peer + DNFTcpSocket + DNFTcpHandler + DNFPacketDecoder + DNFTcpAcceptThread + DNFTcpNetworkThread | 否（独占） |
| server_handler | 20 | DNFServerHandler + DNFManagerServer + DNFServerInterface + LoginLogoutStatistics | 否（独占） |
| app | 19 | DNFApplication + DNFAppStartInit + DNFAppConfig + DNFAppStopInit + DNFServerConfig + DNFKillUserConfig + DNFMonitorServerMain + DNFNetworkThread | 否（独占） |
...
| udp_packet | 12 | DNFUdpHandler + DNFTickHandler + DNFPacketTracer | 否（独占） |
| tables | 28 | DNFPacketBuffer + DNFTableBase + ServerXml + IPCounter + GMAccounts + DNFTowerRank + DNFProhibitUser | 否（独占） |

## 8. 已知提示（本轮基线已含，先看 md 再动手）

- 剩余 377 个绝大多数是 -O0 代码生成差异（栈槽/寄存器/求值顺序/寻址形态/
