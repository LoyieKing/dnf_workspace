# guild 第 29 轮类粒度回归 —— 子 Agent 通用手册（2026-08-12）
...
- **第 29 轮权威基线（父 agent 已从 HEAD 全新重建并全量重生成）**：
  guild 非 identical = **274**（DIFF 256 + NEAR 18；IDENTICAL 959 /
  IDENTICAL_AE 317）。上一轮提交里的 md 集（315）是对陈旧二进制测的；
  HEAD 源码已含全部修复，本轮按 274 起步。
- 你组清单见 `round29_groups/guild/<group>.list`（mangled 名，一行为一个）。
...
3. 重编 + 链接 scratch 二进制（只写你自己的 scratch 目录）：
   ...
   - 私有副本组（pt_a/pt_b/pt_c/guild_a/guild_b，同 TU 多组并行）：
     ...
     scratch_link <group>
     ```
     `/tmp/guild_base/*.cpp` 已由父 agent 刷新为当前 HEAD（含第 28 轮修复），
...
## 7. 分组与 TU 归属（round29_groups/guild/*.list，由第 29 轮权威 manifest 生成）
...
| small_tu | 14 | DNFAppStartInit + DNFTcpSocket + MemoryCashManager + GMAccounts + DNFTcpNetworkThread + DNFNetworkThread + DNFAppStopInit + CashObject + TcpNetSystem | 否（独占） |
...
- 剩余 274 个绝大多数是 -O0 代码生成差异（栈槽/寄存器/求值顺序/寻址形态/
