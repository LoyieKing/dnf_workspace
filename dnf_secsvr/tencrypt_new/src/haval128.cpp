/* haval128.cpp -- HAVAL V.1，3 轮、128 位（Yuliang Zheng 参考实现，BSD 许可）
   按 DWARF 类结构封装：CHaval128::havalInit/Update/Final/Transform3。 */
...
void CHaval128::havalTransform3(unsigned int *E, unsigned char *D, unsigned int *T) {
    unsigned int *W = T;
...
int CHaval128::havalUpdate(unsigned char *dataBuffer, unsigned int dataLength) {
...
int CHaval128::havalFinal(unsigned char *digest) {
    ...
    pad_len = (rmd_len < 118) ? (118 - rmd_len) : (246 - rmd_len);
    {
        ...
        havalUpdate((unsigned char *)padding, pad_len);
    }
    havalUpdate(tail, 10);

    /* tailor：128 位折叠（参考实现 FPTLEN == 128） */
    {
        unsigned int temp;
        temp = (m_pctx->digest[7] & 0x000000FF) |
               (m_pctx->digest[6] & 0xFF000000) |
               (m_pctx->digest[5] & 0x00FF0000) |
               (m_pctx->digest[4] & 0x0000FF00);
        m_pctx->digest[0] += rotate_right(temp, 8);
        temp = (m_pctx->digest[7] & 0x0000FF00) |
               (m_pctx->digest[6] & 0x000000FF) |
               (m_pctx->digest[5] & 0xFF000000) |
               (m_pctx->digest[4] & 0x00FF0000);
        m_pctx->digest[1] += rotate_right(temp, 16);
        temp = (m_pctx->digest[7] & 0x00FF0000) |
               (m_pctx->digest[6] & 0x0000FF00) |
               (m_pctx->digest[5] & 0x000000FF) |
               (m_pctx->digest[4] & 0xFF000000);
        m_pctx->digest[2] += rotate_right(temp, 24);
        temp = (m_pctx->digest[7] & 0xFF000000) |
               (m_pctx->digest[6] & 0x00FF0000) |
               (m_pctx->digest[5] & 0x0000FF00) |
               (m_pctx->digest[4] & 0x000000FF);
        m_pctx->digest[3] += temp;
    }
