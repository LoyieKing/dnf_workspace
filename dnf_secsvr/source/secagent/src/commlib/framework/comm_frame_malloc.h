...
public:
        ...
        for (unsigned int i = 0; i < NUM_OF_FRAMELIST; ++i) {
            ...
            while (pool.size() != 0) {
                ...
                if (proc_frame != 0) {
                    ...
                    Comm_App_Frame::operator delete(
                        proc_frame, Comm_App_Frame::LEN_OF_APPFRAME_HEAD);
                }
            }
        }
        zen_lock_.~MUTEX();
    ...
    void ExtendFrameListCapacity(unsigned int list_no, unsigned int extend_num) { // line 385
        ...
        for (j = 0; j < extend_num; ++j) {
            proc_frame =
                new (size_appframe_[list_no] + 1)
                Comm_App_Frame(0, Comm_App_Frame::LEN_OF_APPFRAME_HEAD,
                               0x1000000);
