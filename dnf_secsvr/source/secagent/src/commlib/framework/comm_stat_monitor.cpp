...
// Original source: /data/secci/ci/jenkins/workspace/g3_release_suse32_bugfix_tag435/src/commlib/framework/comm_stat_monitor.cpp
...
int Comm_Stat_Monitor::get_svr_info_from_stat_fname(const char *stat_file_name, SERVICES_ID *service_info, bool *if_zerg) {
    // local: bool tmp_if_zerg;
    // local: const char *scan_format;
    // local: SERVICES_ID tmp_service_info;
    // local: int ret;
}

void Comm_Stat_Monitor::create_stat_fname(bool if_zerg, const SERVICES_ID &service_info) {
}

int Comm_Stat_Monitor::initialize(bool if_zerg, const SERVICES_ID &service_info, size_t num_stat_item, const ZEN_STATUS_ITEM_WITHNAME *item_ary, bool mutli_thread) {
    // local: int ret;
}
