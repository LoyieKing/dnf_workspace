// 还原自 secagent 二进制（zen_lock_thread_mutex.cpp，GCC 4.1.0 SUSE -O2，2026-08-10）
// ZEN_Thread_Mutex（递归型）/ ZEN_Thread_NONR_Mutex（普通型）/ ZEN_Thread_Recursive_Mutex（递归型）
// 构造：无参；pthread_mutexattr_initex(pshared=0, type=0/1, 忽略位, NULL) + pthread_mutex_init。
...
// 注意（secagent bugfix_tag435 与 gunnersvr 的行为差异）：
//   - 三个构造器均无 mutex_name 参数（传 NULL）。
//   - ZEN_Thread_Mutex 构造在 pthread_mutex_init 成功后才销毁 mutexattr；
//     NONR/Recursive 构造则无条件销毁。
...
ZEN_Thread_NONR_Mutex::ZEN_Thread_NONR_Mutex()
    ...
    int ret = ZEN_OS::pthread_mutexattr_initex(&mutexattr, false, false, true, NULL);
    if (ret != 0) {
        int err = errno;
...
ZEN_Thread_Recursive_Mutex::ZEN_Thread_Recursive_Mutex()
    : ZEN_Lock_Base(0) {
    pthread_mutexattr_t mutexattr;
    int ret = ZEN_OS::pthread_mutexattr_initex(&mutexattr, false, true, true, NULL);
    if (ret != 0) {
        int err = errno;
...
ZEN_Thread_Mutex::ZEN_Thread_Mutex()
    : ZEN_Lock_Base(0) {
    pthread_mutexattr_t mutexattr;
    int ret = ZEN_OS::pthread_mutexattr_initex(&mutexattr, false, true, false, NULL);
    if (ret != 0) {
        int err = errno;
