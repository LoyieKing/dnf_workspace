...
// Original source: /data/secci/ci/jenkins/workspace/g3_release_suse32_bugfix_tag435/src/commlib/zenlib/zen_os_adapt_dirent.cpp
...
int ZEN_OS::rmdir(const char *pathname) {
}

int ZEN_OS::mkdir(const char *pathname, mode_t mode) {
}

int ZEN_OS::mkdir_recurse(const char *pathname, mode_t mode) {
    // local: char process_dir[];
    // local: size_t path_len;
    // local: int ret;
    // local: size_t i;
}

int ZEN_OS::chdir(const char *dirname) {
}

char * ZEN_OS::getcwd(char *buffer, int maxlen) {
}

const char * ZEN_OS::dirname(const char *path_name, char *dir_name, size_t buf_len) {
    // local: const char *temp;
    // local: size_t len;
}

const char * ZEN_OS::basename(const char *path_name, char *file_name, size_t buf_len) {
    // local: const char *temp;
}

int ZEN_OS::alphasort(const dirent **left, const dirent **right) {
}

void ZEN_OS::free_scandir_result(int list_number, dirent **namelist) {
    // local: int i;
}

int ZEN_OS::scandir(const char *dirname, dirent ***namelist, int (**selector)(const dirent *arg0), int (**comparator)(const dirent **arg0, const dirent **arg1)) {
}

int ZEN_OS::readdir_r(DIR *dir_handle, dirent *entry, dirent **result) {
}

dirent * ZEN_OS::readdir(DIR *dir_handle) {
}

int ZEN_OS::closedir(DIR *dir_handle) {
}

DIR * ZEN_OS::opendir(const char *dir_name) {
}
