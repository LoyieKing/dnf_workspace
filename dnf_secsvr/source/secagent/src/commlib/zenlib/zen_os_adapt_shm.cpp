...
// Original source: /data/secci/ci/jenkins/workspace/g3_release_suse32_bugfix_tag435/src/commlib/zenlib/zen_os_adapt_shm.cpp
...
int ZEN_OS::shmctl(ZEN_HANDLE shmid, int cmd, shmid_ds *buf) {
}

int ZEN_OS::shmdt(const void *shmaddr) {
}

void * ZEN_OS::shmat(ZEN_HANDLE shmid, const void *shmaddr, int shmflg) {
}

ZEN_HANDLE ZEN_OS::shmget(key_t sysv_key, size_t size, int oflag) {
}

int ZEN_OS::shm_stat(char *mmap_file_name, zen_os_stat *buf) {
    // local: char shm_file_name[];
}

int ZEN_OS::shm_unlink(const char *file_path) {
    // local: char shm_file_name[];
    // local: int shm_path_head_len;
}

ZEN_HANDLE ZEN_OS::shm_open(const char *file_path, int mode, mode_t perms) {
    // local: char shm_file_name[];
    // local: int shm_path_head_len;
}

int ZEN_OS::munmap(void *addr, size_t len) {
}

int ZEN_OS::msync(void *addr, size_t len, int sync) {
}

int ZEN_OS::mprotect(const void *addr, size_t len, int prot) {
}

void * ZEN_OS::mmap(void *addr, size_t len, int prot, int flags, ZEN_HANDLE file_handle, size_t off) {
}
