...
// 以下薄封装原为头文件内联（二进制无 out-of-line 符号）
inline int ZEN_Bus_MMAPPipe::push_back_bus(size_t index, const ZEN_LIB::dequechunk_node *node) {
    return bus_pipe_pointer_[index]->push_end(node);
}

inline int ZEN_Bus_MMAPPipe::pop_front_bus(size_t index, ZEN_LIB::dequechunk_node *node) {
    return bus_pipe_pointer_[index]->pop_front(node);
}

inline int ZEN_Bus_MMAPPipe::read_front_bus(size_t index, ZEN_LIB::dequechunk_node *&node) {
    return bus_pipe_pointer_[index]->read_front_new(node);
}

inline int ZEN_Bus_MMAPPipe::pop_front_bus(size_t index) {
    return bus_pipe_pointer_[index]->discard_frond();
}

inline int ZEN_Bus_MMAPPipe::get_front_nodesize(size_t index, size_t &nodesize) {
    ZEN_LIB::dequechunk_node *node = NULL;
    int ret = bus_pipe_pointer_[index]->read_front_new(node);
    if (ret == 0 && node != NULL) {
        nodesize = node->size_of_node_;
    }
    return ret;
}

inline bool ZEN_Bus_MMAPPipe::is_full_bus(size_t index) {
    return bus_pipe_pointer_[index]->full();
}

inline bool ZEN_Bus_MMAPPipe::is_empty_bus(size_t index) {
    return bus_pipe_pointer_[index]->empty();
}

inline void ZEN_Bus_MMAPPipe::get_bus_freesize(size_t index, size_t &freesize, size_t &size) {
    freesize = bus_pipe_pointer_[index]->freesize();
    size = bus_head_.size_of_room_[index];
}
