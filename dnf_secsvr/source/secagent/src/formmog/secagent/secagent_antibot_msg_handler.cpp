...
// line 1520
unsigned int SecagentAntibotMsgHandler::rpcode_hash(const unsigned char *offset, unsigned int len) {
    // local: unsigned int tmp_hash;
    // local: const unsigned char *ptmp;
    // local: unsigned int i;
}

// line 1494
int SecagentAntibotMsgHandler::proc_slx_share_info_msg_ts(const AntibotSanlixData *sanlix_data, /*anon struct*/ int *antibot_pkg) {
}
...
// line 1372
int SecagentAntibotMsgHandler::proc_slx_share_info_msg_te(const AntibotSanlixData *sanlix_data, /*anon struct*/ int *antibot_pkg) {
}

// line 1320
int SecagentAntibotMsgHandler::proc_tenparty_data_msg(const AntibotSanlixData *sanlix_data, /*anon struct*/ int *antibot_pkg) {
}

// line 1345
int SecagentAntibotMsgHandler::proc_scheme_data_msg(const AntibotSanlixData *sanlix_data, /*anon struct*/ int *antibot_pkg) {
}

// line 1106
void get_quickly_func(unsigned int *idx, const DibData *cdib_data) {
    // local: unsigned int i;
    // local: unsigned int quickly_idx;
    // local: const unsigned int dib_count;
    // local: const unsigned int s_quickly_tencrypt_ids[];
}

int Comm_App_Frame::appdata_encode<sec_proto::SendToSDK>(unsigned int szframe_appdata, const /*anon struct*/ int &info, unsigned int data_start, size_t *sz_code) {
    // local: size_t use_len;
    // local: int ret;
}

int SecAgentGamesvrChannel::send<sec_proto::SendToSDK>(unsigned int uin, unsigned int cmd, unsigned int appid, unsigned int channel_id, const /*anon struct*/ int &pkg) {
    // local: size_t len;
    // local: Comm_App_Frame *frame;
}

// line 1072
int SecagentAntibotMsgHandler::send_buff_to_client(const char *buffer, size_t buf_len, unsigned int channel_id, unsigned int room_id) {
    // local: /*anon struct*/ int send_to_sdk;
}

// line 358
int SecagentAntibotMsgHandler::pack_secsvr_antibot_to_frame(Comm_App_Frame *recv_frame, /*anon struct*/ int *sdk_pkg, const /*anon struct*/ int &antibot_pkg) {
    // local: size_t use_len;
    // local: int ret;
}

// line 952
int SecagentAntibotMsgHandler::send_antibot_msg(const /*anon struct*/ int &antibot_pkg, unsigned int channel_id, unsigned int room_id) {
    // local: char buffer[];
    // local: unsigned int buf_len;
    // local: size_t use_len;
    // local: int ret;
    // local: unsigned int max_package_size;
    // local: char piece_data[];
    // local: int piece_size;
    // local: char *temp_ptr;
    // local: /*anon struct*/ int head;
    // local: size_t piece_used_len;
    // local: short unsigned int piece_count;
    // local: unsigned int crc32_code;
    // local: short unsigned int i;
    // local: char *temp_piece_ptr;
    // local: int temp_piece_size;
}

// line 1239
int SecagentAntibotMsgHandler::send_testpkg() {
    // local: const SecagentGameExe *game_exe;
    // local: unsigned int exe_data_size;
    // local: const unsigned char *exe_data;
    // local: /*anon struct*/ int send_msg;
    // local: int rand_start;
}

// line 585
int SecagentAntibotMsgHandler::proc_ab_rpcode_pkg(/*anon struct*/ int *sdk_pkg) {
    // local: int ret;
    // local: /*anon struct*/ int rpcode_pkg;
    // local: size_t use_len;
    // local: unsigned int hash_val;
}

int Comm_App_Frame::appdata_encode<sec_proto::ReportErrReq>(unsigned int szframe_appdata, const /*anon struct*/ int &info, unsigned int data_start, size_t *sz_code) {
    // local: size_t use_len;
    // local: int ret;
}

// line 302
int SecagentAntibotMsgHandler::encrypt(Comm_App_Frame *recv_frame, /*anon struct*/ int *sdk_pkg) {
    // local: unsigned int func_id;
    // local: unsigned char *crypt_key;
    // local: /*anon struct*/ int antibot_pkg;
    // local: int ret;
}

int Comm_App_Frame::appdata_encode<sec_proto::RecvDataFromSDKReq>(unsigned int szframe_appdata, const /*anon struct*/ int &info, unsigned int data_start, size_t *sz_code) {
    // local: size_t use_len;
    // local: int ret;
}
...
// line 452
int SecagentAntibotMsgHandler::proc_secsvr_ab_other_msg(uint16_t antibot_cmd, Comm_App_Frame *recv_frame, /*anon struct*/ int *sdk_pkg) {
    // local: int ret;
}

// line 1398
int SecagentAntibotMsgHandler::proc_sanlix_data_msg(uint16_t antibot_cmd, Comm_App_Frame *recv_frame, /*anon struct*/ int *sdk_pkg) {
    // local: /*anon struct*/ int antibot_pkg;
    // local: int ret;
    // local: const AntibotSanlixData *sanlix_data;
    // local: const uint16_t scheme_id;
    // local: const uint16_t sub_proto_id;
...
// line 112
int SecagentAntibotMsgHandler::proc_secsvr_antibot_msg(Comm_App_Frame *recv_frame, /*anon struct*/ int *sdk_pkg) {
    // local: int ret;
    // local: const uint16_t antibot_cmd;
    // local: short unsigned int __v;
    // local: short unsigned int __x;
}
...
// line 1140
int SecagentAntibotMsgHandler::get_crypt_id() {
    // local: const DibData *cdib_data;
    // local: unsigned int idx;
}
...
int SecagentAntibotMsgHandler::send_cmd_dib_key(SecAgentUserInfo *user_info) {
    ...
    // local: /*anon struct*/ int send_msg;
    // local: int dib_index;
    // local: const DibItem *dib_item;
    // local: int rang_index;
    // local: const DibKeyInfo *key_range_item;
    // local: int offset;
}

// line 530
int SecagentAntibotMsgHandler::send_error_pkg_to_secsvr(const /*anon struct*/ int &sdk_pkg, unsigned int error_id) {
    // local: /*anon struct*/ int report_err_pkg;
...
// line 1270
int SecagentAntibotMsgHandler::proc_ab_de_log(const /*anon struct*/ int &sdk_pkg) {
    // local: /*anon struct*/ int antibot_pkg;
    // local: int ret;
}

// line 1153
int SecagentAntibotMsgHandler::proc_ab_de_testpkg(const /*anon struct*/ int &sdk_pkg) {
    // local: int ret;
    // local: /*anon struct*/ int antibot_pkg;
    // local: unsigned int tmp_crc;
}

// line 708
int SecagentAntibotMsgHandler::proc_ab_init_pkg(const /*anon struct*/ int &sdk_pkg) {
    // local: /*anon struct*/ int antibot_pkg;
    // local: int ret;
    // local: const SecagentGameExe *game_exe;
    // local: const unsigned char *exe_data;
...
// line 476
int SecagentAntibotMsgHandler::proc_tsssdk_ab_other_msg(uint16_t antibot_cmd, const /*anon struct*/ int &sdk_pkg) {
    // local: int ret;
}

// line 388
int SecagentAntibotMsgHandler::proc_ab_ready_pkg(const /*anon struct*/ int &sdk_pkg) {
    // local: int ret;
    // local: /*anon struct*/ int antibot_pkg;
    // local: unsigned char *key_buff;
    // local: uint32_t temp_crc;
}

// line 210
int SecagentAntibotMsgHandler::decrypt(/*anon struct*/ int *sdk_pkg, uint16_t *real_antibot_cmd) {
    // local: unsigned int dib_id;
    // local: unsigned char *crypt_key;
    // local: /*anon struct*/ int antibot_pkg;
    // local: int ret;
    // local: short unsigned int __v;
...
// line 22
int SecagentAntibotMsgHandler::proc_tsssdk_antibot_msg(Comm_App_Frame *recv_frame, /*anon struct*/ int *sdk_pkg) {
    // local: int ret;
    // local: uint16_t antibot_cmd;
    // local: short unsigned int __v;
    // local: short unsigned int __x;
}
