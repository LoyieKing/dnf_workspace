...
// Original source: /data/secci/ci/jenkins/workspace/g3_release_suse32/src/commsvr/gunnersvr/gunner_reporter.cpp
...
#include "output/commlib/framework/release/include/comm_sndrcv_base.h"
...
#include "src/commsvr/gunnersvr/<built-in>"
#include <_G_config.h>
#include <algorithm>
#include <alloca.h>
#include <arpa/inet.h>
...
void std::__uninitialized_fill_n_aux<sec_proto::MonitorSvrReport*, unsigned int, sec_proto::MonitorSvrReport>(/*anon struct*/ int *__first, unsigned int __n, const /*anon struct*/ int &__x, __false_type arg3) {
    // local: /*anon struct*/ int *__cur;
}

void std::__uninitialized_fill_n_a<sec_proto::MonitorSvrReport*, unsigned int, sec_proto::MonitorSvrReport, sec_proto::MonitorSvrReport>(/*anon struct*/ int *__first, unsigned int __n, const /*anon struct*/ int &__x, /*anon struct*/ int arg3) {
}
...
// line 154
int GunnerInfoReporter::pack_by_appid(const GunnerStatInfo &stat_info, unsigned int appid_num, const unsigned int *appid, vector<sec_proto::MonitorSvrReport,std::allocator<sec_proto::MonitorSvrReport> > &report_pkgs) {
    // local: time_t tmp_remain_time;
    // local: uint32_t report_time;
    // local: unsigned int array_size;
...
int GunnerInfoReporter::report_statinfo_to_monitor(const GunnerStatInfo &stat_info) {
    // local: int ret;
    // local: const int APPID_NUM;
    // local: unsigned int appid_num;
    // local: unsigned int appid[];
    // local: /*anon struct*/ int report_pkgs;
    // local: unsigned int i;
    // local: unsigned int j;
    // local: unsigned int i;
}
...
int GunnerInfoReporter::report(time_t report_time) {
    // local: /*anon struct*/ int stat_files;
    // local: int ret;
    // local: unsigned int size;
    // local: GunnerStatInfo stat_info;
    // local: unsigned int i;
    // local: time_t cost_time;
}
