...
// Original source: /data/secci/ci/jenkins/workspace/g3_release_suse32/src/commsvr/gunnersvr/gunner_channel_mgr.cpp
...
int Comm_App_Frame::appdata_encode<sec_proto::MonitorSvrReport>(unsigned int szframe_appdata, const /*anon struct*/ int &info, unsigned int data_start, size_t *sz_code) {
    // local: size_t use_len;
    // local: int ret;
}

int Zulu_SendRecv_Package::send_svc_package<sec_proto::MonitorSvrReport>(unsigned int qq_uin, unsigned int cmd, const /*anon struct*/ int &info, ZEN_Time_Value *time_wait, unsigned int app_id, unsigned int backfill_trans_id) {
    // local: int ret;
}

// line 109
int GunnerChannel::send_to_monitor(/*anon struct*/ int pkg) {
}

// line 71
int GunnerChannel::connect_monitor() {
    // local: SERVICES_ID selfsvr;
    // local: SERVICES_ID recvsvc;
...
// line 64
int GunnerChannel::init(const /*anon struct*/ int &monitor_info) {
}

// line 4
int GunnerChannelMgr::init_channels(GunnerChannelMgr::LPCONFIG config) {
    // local: int ret;
    // local: unsigned int i;
}
