...
// Original source: /data/secci/ci/jenkins/workspace/g3_release_suse32/src/commsvr/gunnersvr/gunner_tool.cpp
...
void GunnerTool::print_update_time() {
    // local: unsigned int report_time;
    // local: char time_str[];
    // local: timeval tv;
}
...
int GunnerTool::report_gunner_info() {
    // local: GunnerStatInfo gunner_info;
    // local: ZEN_STATUS_ITEM_WITHNAME report_item;
    // local: timeval now;
    // local: GunnerChannelMgr::LPCONFIG config;
    // local: int ret;
}
