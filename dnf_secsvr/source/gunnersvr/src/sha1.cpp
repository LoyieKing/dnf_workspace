/* sha1.cpp -- Steve Reid 公版 SHA-1（RFC 3174）的 tencrypt_new 类封装
   由 DWARF 桩重建：CSha1::SHA1_Init/Update/Final/Transform，局部变量名与二进制一致。 */

#include "inc/sha1.h"

#include <string.h>

/* 公版声明（Steve Reid <sreid@sea-to-sky.net>）：本代码允许任何目的自由使用。 */
...
/* blk0/blk1 采用小端取数（i586） */
#define blk0(i) (block->l[i] = (rol(block->l[i], 24) & 0xFF00FF00) | \
                 (rol(block->l[i], 8) & 0x00FF00FF))
#define blk1(i) (block->l[i & 15] = rol(block->l[(i + 13) & 15] ^ \
...
#define R1(v, w, x, y, z, i) \
    z += ((w & (x ^ y)) ^ y) + blk1(i) + 0x5A827999 + rol(v, 5); \
    w = rol(w, 30);
#define R2(v, w, x, y, z, i) \
    z += (w ^ x ^ y) + blk1(i) + 0x6ED9EBA1 + rol(v, 5); \
    w = rol(w, 30);
#define R3(v, w, x, y, z, i) \
    z += (((w | x) & y) | (w & x)) + blk1(i) + 0x8F1BBCDC + rol(v, 5); \
    w = rol(w, 30);
#define R4(v, w, x, y, z, i) \
    z += (w ^ x ^ y) + blk1(i) + 0xCA62C1D6 + rol(v, 5); \
    w = rol(w, 30);

void CSha1::SHA1Transform(unsigned int *state, unsigned char *buffer) {
    unsigned int a, b, c, d, e;
    CHAR64LONG16 *block;
    unsigned char workspace[80];
