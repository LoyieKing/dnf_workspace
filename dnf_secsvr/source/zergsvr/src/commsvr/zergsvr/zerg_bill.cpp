...
// Original source: /data/secci/ci/jenkins/workspace/g3_release_suse32_bugfix_tag296/src/commsvr/zergsvr/zerg_bill.cpp
...
#include "src/commsvr/zergsvr/zerg_bill.h"
#include <algorithm>
#include <alloca.h>
#include <arpa/inet.h>
...
Zerg_Bill * Zerg_Bill::_instance = NULL;
...
// line 38
void Zerg_Bill::write_get_svrinfo_bill(const char *svrinfo, SERVICES_ID req_service, unsigned int idc_no, unsigned int server_guid_no) {
    g_bill << ConstDataSource(0xbf69) << LogType(1) << LogComment("get svr info") << svrinfo
           << (long long)req_service.services_type_ << (long long)req_service.services_id_ << endl;
}

// line 17
void Zerg_Bill::clean_instance() {
    if (_instance != NULL) {
        delete _instance;
    }
    _instance = NULL;
...
// line 7
Zerg_Bill * Zerg_Bill::instance() {
    if (_instance == NULL) {
        _instance = new Zerg_Bill();
    }
    return _instance;
}
