...
void Zerg_Comm_Manager::check_freamcount(unsigned int now) {
    // local: unsigned int interval;
    // local: unsigned int frame_per_sec;
}
...
int Zerg_Comm_Manager::get_config(const Zerg_MMAP_BusPipe::ZERG_CONFIG &config) {
    // local: int ret;
    // local: const size_t LEN_TMP_BUFFER;
    // local: char err_buf[];
    // local: size_t i;
}
...
int Zerg_Comm_Manager::check_safeport(ZEN_Sockaddr_In &inetadd) {
    // local: const short unsigned int UNSAFE_PORT1;
    // local: const short unsigned int UNSAFE_PORT2;
    // local: const short unsigned int UNSAFE_PORT3;
    // local: const short unsigned int UNSAFE_PORT4;
    // local: const short unsigned int SAFE_PORT1;
}

void _M_insert_aux(__normal_iterator<TCP_Accept_Handler**,std::vector<TCP_Accept_Handler*, std::allocator<TCP_Accept_Handler*> > > __position, TCP_Accept_Handler *const &__x) {
    // local: TCP_Accept_Handler *__x_copy;
...
int Zerg_Comm_Manager::init_socketpeer(ZERG_SERVICES_INFO &init_svcid) {
    // local: int ret;
    // local: UDP_Svc_Handler *tmp_udphdl;
    // local: TCP_Accept_Handler *tmp_acceptor;
}
