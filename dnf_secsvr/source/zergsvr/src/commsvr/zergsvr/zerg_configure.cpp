...
// line 73
int Zerg_Server_Config::GetServicesIPInfo(const SERVICES_ID &svc_info, ZEN_Sockaddr_In &ipaddr) {
    // local: int ret;
    // local: unsigned int idc_no;
    // local: char ip_info[];
    // local: unsigned int server_guid_no;
    // local: int num;
}

void _M_insert_aux(__normal_iterator<ZERG_SERVICES_INFO*,std::vector<ZERG_SERVICES_INFO, std::allocator<ZERG_SERVICES_INFO> > > __position, const ZERG_SERVICES_INFO &__x) {
    // local: ZERG_SERVICES_INFO __x_copy;
...
int Zerg_Server_Config::init(const Zerg_MMAP_BusPipe::ZERG_CONFIG *config) {
    ...
    // local: unsigned int slave_num;
    // local: unsigned int i;
    // local: ZERG_SERVICES_INFO slave_svc_info;
}
