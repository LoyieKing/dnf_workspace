...
// Original source: /data/secci/ci/jenkins/workspace/g3_release_suse32_bugfix_tag296/src/commlib/zenlib/zen_os_adapt_sysinfo.cpp
...
int ZEN_OS::get_system_info(ZEN_SYSTEM_INFO *zen_system_info) {
    // local: sysinfo info;
    // local: int ret;
}

int ZEN_OS::read_proc_get_uptime(ZEN_SYSTEM_PERFORMANCE *info) {
    // local: const char *PROC_FILENAME_UPTIME;
    // local: char buffer[];
    // local: size_t read_len;
    // local: int ret;
    // local: const char *in_para;
    // local: char *out_para;
    // local: double uptime;
    // local: const uint32_t SEC_PER_USEC;
}

int ZEN_OS::read_proc_get_stat(ZEN_SYSTEM_PERFORMANCE *info) {
    // local: const char *PROC_FILENAME_STAT;
    // local: char buffer[];
    // local: size_t read_len;
    // local: int ret;
    // local: const char *in_para;
    // local: char *out_para;
    // local: long int cpu_tick_precision;
    // local: long int cpu_config_num;
    // local: const uint64_t SEC_PER_USEC;
    // local: uint64_t time_data;
}

int ZEN_OS::read_fun_get_systemperf(ZEN_SYSTEM_PERFORMANCE *zen_system_perf) {
    // local: sysinfo info;
    // local: int ret;
}

int ZEN_OS::read_proc_get_systemperf(ZEN_SYSTEM_PERFORMANCE *info) {
    // local: int ret;
}

int ZEN_OS::get_system_performance(ZEN_SYSTEM_PERFORMANCE *zen_system_perf) {
}
