...
// Original source: /data/secci/ci/jenkins/workspace/g3_release_suse32_bugfix_tag296/src/commlib/zenlib/zen_config_ini_implement.cpp
// Compiler: GNU C++ 4.1.0 (SUSE Linux)
// zergsvr 与 gunnersvr 的函数体逐字节一致（仅类名 Zen_INI_PropertyTree -> ZenINIPropertyTree、
// read 参数 Zen_Conf_PropertyTree -> ZenConfPropertyTree）。
...
int ZEN_INI_Implemention::read(const char *file_name, ZenConfPropertyTree &propertytree)
{
...
int ZenINIPropertyTree::get_string_value(const std::string &sectionname, const std::string &keyname, std::string &value) const
{
    return get_leaf_str(sectionname, keyname, value);
}

int ZenINIPropertyTree::get_string_value(const std::string &sectionname, const std::string &keyname, char *buf, size_t size_buf) const
{
    return get_leaf_str(sectionname, keyname, buf, size_buf);
}

int ZenINIPropertyTree::get_int32_value(const std::string &sectionname, const std::string &keyname, int32_t &value) const
{
    return get_leaf_int(sectionname, keyname, value);
}

int ZenINIPropertyTree::get_uint32_value(const std::string &sectionname, const std::string &keyname, uint32_t &value) const
{
    return get_leaf_uint(sectionname, keyname, value);
}

int ZenINIPropertyTree::get_bool_value(const std::string &sectionname, const std::string &keyname, bool &value) const
{
