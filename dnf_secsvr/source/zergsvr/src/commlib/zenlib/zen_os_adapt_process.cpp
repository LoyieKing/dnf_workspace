...
// Original source: /data/secci/ci/jenkins/workspace/g3_release_suse32_bugfix_tag296/src/commlib/zenlib/zen_os_adapt_process.cpp
...
sighandler_t ZEN_OS::signal(int signum, sighandler_t func) {
}

char * ZEN_OS::getenv(const char *name) {
}
...
pid_t ZEN_OS::getppid() {
}

pid_t ZEN_OS::getpid() {
}

int ZEN_OS::get_process_perf(pid_t process_id, ZEN_PROCESS_PERFORM *prc_perf_info) {
    // local: int ret;
}

int ZEN_OS::get_self_perf(ZEN_PROCESS_PERFORM *prc_perf_info) {
    // local: pid_t process_id;
}
