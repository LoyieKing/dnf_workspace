...
// Original source: /data/secci/ci/jenkins/workspace/g3_release_suse32_bugfix_tag296/src/commlib/framework/comm_mml_console_handler.cpp
...
int MML_Console_Handler::initialize(const Zerg_MMAP_BusPipe::ZERG_CONFIG &config) {
    // local: int ret;
}

int MML_Console_Handler::read_config(const Zerg_MMAP_BusPipe::ZERG_CONFIG &config) {
    // local: int ret;
    // local: const unsigned int TMP_BUF_LEN;
    // local: char tmp_tip[];
}

ZEN_SOCKET MML_Console_Handler::get_handle() {
}

int MML_Console_Handler::handle_input() {
    // local: int ret;
    // local: ssize_t ssret;
    // local: size_t str_len;
    // local: bool bret;
}
