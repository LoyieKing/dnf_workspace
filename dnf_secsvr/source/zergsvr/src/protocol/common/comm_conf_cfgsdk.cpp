...
// Original source: /data/secci/ci/jenkins/workspace/g3_release_suse32_bugfix_tag296/src/protocol/common/comm_conf_cfgsdk.cpp
...
tsf4g_tdr::TdrError::ErrorType entryFromXml(/*anon struct*/ int &reader, unsigned int cutVer) {
    ...
    // local: const char *value4game_id_;
    // local: const char *value4idc_no_;
    // local: const char *value4world_;
}

// line 703
tsf4g_tdr::TdrError::ErrorType entryFromXml(/*anon struct*/ int &reader, unsigned int cutVer) {
    // local: tsf4g_tdr::TdrError::ErrorType ret;
    // local: const char *value4slave_cfgsvr_type_;
    // local: const char *value4slave_cfgsvr_id_;
    // local: const char *value4slave_cfgsvr_ip_;
    // local: const char *value4slave_cfgsvr_port_;
    // local: const char *value4game_id_;
    // local: const char *value4idc_no_;
    // local: const char *value4world_;
    // local: const char *value4deploy_info_num_;
    // local: const size_t length4slave_cfgsvr_ip_;
    // local: uint32_t deploy_info__i;
}
