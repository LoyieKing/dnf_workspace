...
tsf4g_tdr::TdrError::ErrorType StartSynCfgFileRsp::pack(tsf4g_tdr::TdrWriteBuf& destBuf, unsigned int cutVer) const {
    TdrError::ErrorType ret = TdrError::TDR_NO_ERROR;
    ret = cfg_storage_info_.pack(destBuf, 1);
    if (ret != TdrError::TDR_NO_ERROR) return ret;
    ret = destBuf.writeUInt32((uint32_t)cfg_info_index_);
    if (ret != TdrError::TDR_NO_ERROR) return ret;
    return TdrError::TDR_NO_ERROR;
}
...
tsf4g_tdr::TdrError::ErrorType StartSynCfgFileRsp::unpack(tsf4g_tdr::TdrReadBuf& destBuf, unsigned int cutVer) {
    TdrError::ErrorType ret = TdrError::TDR_NO_ERROR;
    ret = cfg_storage_info_.unpack(destBuf, 1);
    if (ret != TdrError::TDR_NO_ERROR) return ret;
    ret = destBuf.readUInt32((uint32_t&)cfg_info_index_);
    if (ret != TdrError::TDR_NO_ERROR) return ret;
    return TdrError::TDR_NO_ERROR;
}
...
tsf4g_tdr::TdrError::ErrorType StartSynCfgFileRsp::visualize(tsf4g_tdr::TdrWriteBuf& destBuf, int indent, char separator) const {
    TdrError::ErrorType ret = TdrError::TDR_NO_ERROR;
    ret = TdrBufUtil::printVariable(destBuf, indent, separator, "[cfg_storage_info_]", true);
    if (ret != TdrError::TDR_NO_ERROR) return ret;
    if (indent >= 0) {
        ret = cfg_storage_info_.visualize(destBuf, indent + 1, separator);
    } else {
        ret = cfg_storage_info_.visualize(destBuf, indent, separator);
    }
    if (ret != TdrError::TDR_NO_ERROR) return ret;
    ret = TdrBufUtil::printVariable(destBuf, indent, separator, "[cfg_info_index_]", "%d", cfg_info_index_);
    if (ret != TdrError::TDR_NO_ERROR) return ret;
    return TdrError::TDR_NO_ERROR;
}
